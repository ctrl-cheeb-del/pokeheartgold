...
BOOL ov12_0223A5E4(OverlayManager *man) {
    Ov12Args *args = OverlayManager_GetArgs(man);
    Ov12HeapGroup *group = args->heapGroup;
    int i;
    if (group->enabled) {
        for (i = 0; i < 4; i++) {
            if (group->allocations[i] != NULL) {
                Heap_Free(group->allocations[i]);
            }
        }
        Heap_Free(group);
        return TRUE;
...
void ov12_0223A620(Ov12TextData *data) {
    if ((data->input & 0x404) != 0 || (data->flags & 0x10) != 0) {
