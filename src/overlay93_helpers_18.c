...
void ov93_02262444(void *sys, void **out) {
    s32 i;
    for (i = 0; i < 2; i++) {
        void *sp = SpriteSystem_NewSprite(PAT(sys, 0x24), PAT(sys, 0x28), &ov93_02262F38);
        ManagedSprite_SetAnim(sp, 0x1b);
        ManagedSprite_SetOamMode(sp, 1);
        ManagedSprite_SetDrawFlag(sp, 0);
        Sprite_TickFrame(PAT(sp, 0));
        *out++ = sp;
    }
}
