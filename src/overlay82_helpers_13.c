...
u32 ov82_0223FC9C(Ov82Spr *spr) {
    Sprite_Delete(spr->sprite);
    Heap_Free(spr);
    return 0;
}

void ov82_0223FCB0(Ov82Spr *spr, BOOL flag) {
    Sprite_SetDrawFlag(spr->sprite, flag);
}

void ov82_0223FCBC(Ov82Spr *spr, s32 x, s32 y) {
    VecFx32 pos;
    if (spr->animPtr != NULL) {
        Sprite_TryChangeAnimSeq(spr->sprite, spr->animPtr[0]);
    }
    pos = *Sprite_GetMatrixPtr(spr->sprite);
    pos.x = x << 12;
    pos.y = y << 12;
    Sprite_SetMatrix(spr->sprite, &pos);
}

void ov82_0223FCFC(Ov82Spr *spr, u32 seq) {
    Sprite_SetAnimSpeed(spr->sprite, FX32_ONE);
    Sprite_TryChangeAnimSeq(spr->sprite, seq);
}

void ov82_0223FD18(Ov82Spr *spr, u32 species) {
    Sprite_SetPalOffsetRespectVramOffset(spr->sprite, Pokemon_GetIconPalette(species));
}

void ov82_0223FD2C(BgConfig *bgConfig, Window *wins) {
    u8 i;
    for (i = 0; i < 4; i++) {
        AddWindow(bgConfig, &wins[i], &ov82_0223FF00[i]);
        FillWindowPixelBuffer(&wins[i], 0);
    }
}
...
void ov82_0223FD78(Window *win, u32 frame) {
    LoadUserFrameGfx2(win->bgConfig, GetWindowBgId(win), 0x3D9, 0xa, (u8)frame, HEAP_ID_105);
    FillWindowPixelBuffer(win, 0xf);
    DrawFrameAndWindow2(win, 0, 0x3D9, 0xa);
}

YesNoPrompt *ov82_0223FDB8(enum HeapID heapId) {
    return YesNoPrompt_Create(heapId);
}

void ov82_0223FDC0(YesNoPrompt *p) {
    YesNoPrompt_Destroy(p);
}
