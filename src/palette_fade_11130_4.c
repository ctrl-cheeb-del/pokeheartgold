...
void sub_02011B5C(EffectContext *ctx, const void *config) {
    void *work = Heap_Alloc(ctx->f[8], 0x30);
    ctx->f[5] = (u32)work;
    sub_02011BF0((void *)ctx->f[5], config, ctx->f[1], ctx->f[2], ctx->f[4], ctx->f[6], ctx->f[7], ctx->f[8]);
    ctx->f[3]++;
}

BOOL sub_02011B94(EffectContext *ctx) {
    int result = FALSE;
    EffectWork *work = (EffectWork *)ctx->f[5];
    switch (ctx->f[3]) {
    case 1:
        if (sub_02011CB8(work) == 1) {
            sub_02010F34(work->f[8], work->f[9], ctx->f[4]);
            ctx->f[3]++;
        }
        break;
    case 2:
