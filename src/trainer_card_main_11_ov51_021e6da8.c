...
void ov51_021E6DA8(TrainerCardMainState *state, int update) {
    if (update != 0) {
        if (*(void **)&state->data[0x30f4] == NULL) {
            if (state->data[0x3439] == 0) {
                ov51_021E757C(&state->data[4], *(void **)&state->data[0xe8], *(void **)&state->data[0x33c8]);
                ov51_021E7664(&state->data[0x54], 1, *(void **)&state->data[0x33cc]);
            } else if (state->data[0x3439] == 0xf) {
                ov51_021E7664(&state->data[0x54], 0, *(void **)&state->data[0x33cc]);
            }
        }
        state->data[0x3439] = (state->data[0x3439] + 1) % 30;
    }
}

void ov51_021E6E10(TrainerCardMainState *state, int mode) {
    if (mode == 1) {
        ov51_021E7D44((TrainerCardMainState *)&state->data[0x311c], 1, 2, 0);
        ov51_021E7D44((TrainerCardMainState *)&state->data[0x311c], 0, 0, 0);
    } else {
        ov51_021E7D44((TrainerCardMainState *)&state->data[0x311c], 1, 2, 1);
        ov51_021E7D44((TrainerCardMainState *)&state->data[0x311c], 0, 0, 1);
    }
    ov51_021E78D0(state, mode);
}
