...
void ov96_021F4790(u8 *p) {
    Ov96R58Ui *ui = (Ov96R58Ui *)p;
    u8 i;
    for (i = 0; i < 2; i++) {
        ov96_021EB52C(ui->first[i], 1, 0);
    }
    for (i = 0; i < 4; i++) {
        ov96_021EB52C(ui->second[i], 1, 0);
        ov96_021EB52C(ui->third[i], 1, 0);
    }
    ov96_021EB52C(ui->single, 1, 0);
}

BOOL ov96_021F47F0(u8 *p, int next) {
    if (ov96_021F4700(p, next)) {
        ov96_021F4724(p);
        return TRUE;
    }
    return FALSE;
