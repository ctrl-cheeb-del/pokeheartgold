...
BOOL ov80_0222E05C(void *c) {
    sub_0209680C(APP(c));
    return U16(c, 0x78) == 1;
}

int FrtCmd_072(void *c) {
    u16 n = FrontierScriptContext_ReadHalfWord(c);
    GameStats_Inc(Save_GameStats_Get(PTR(Frontier_GetLaunchArgs(APP(c)), 8)), n);
    return 0;
}

int FrtCmd_073(void *c) {
    u16 n = FrontierScriptContext_ReadHalfWord(c), v = FrontierScript_ReadVar(c);
    GameStats_Add(Save_GameStats_Get(PTR(Frontier_GetLaunchArgs(APP(c)), 8)), n, v);
    return 0;
}

int FrtCmd_074(void *c) {
    u16 n = FrontierScriptContext_ReadHalfWord(c);
    GameStats_AddScore(Save_GameStats_Get(PTR(Frontier_GetLaunchArgs(APP(c)), 8)), n);
    return 0;
}

int FrtCmd_075(void *c) {
    u16 *out = FrontierScript_ReadVarPtr(c);
    *out = sub_0205C298(PTR(Frontier_GetLaunchArgs(APP(c)), 8));
    return 0;
}

int FrtCmd_113(void *c) {
    u16 *out = FrontierScript_ReadVarPtr(c);
    *out = (u16)(u32)sub_020304B4();
    return 0;
}

int FrtCmd_183(void *c) {
    void *p = sub_0202C6F4(PTR(Frontier_GetLaunchArgs(APP(c)), 8));
    sub_0202C4B0(p, ov00_021E6EBC());
    return 0;
}

int FrtCmd_200(void *c) {
    sub_020270C4(PTR(Frontier_GetLaunchArgs(APP(c)), 8));
    FrontierScript_ReadVar(c);
    return 0;
}

int FrtCmd_201(void *c) {
    sub_020378E4(FrontierScript_ReadVar(c));
    return 0;
}

int FrtCmd_203(void *c) {
    u16 v = FrontierScript_ReadVar(c);
    U16(c, 0x78) = v;
    sub_02037AC0((u8)v);
    FrontierScriptContext_Pause(c, ov80_0222E198);
    return 1;
}
