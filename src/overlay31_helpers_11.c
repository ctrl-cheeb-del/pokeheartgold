...
void ov31_0225EDA0(Overlay31State *state) {
    ClearWindowTilemapAndScheduleTransfer(&state->data[0xe4]);
    ClearWindowTilemapAndScheduleTransfer(&state->data[0xf4]);
    ClearWindowTilemapAndScheduleTransfer(&state->data[0x104]);
    ClearWindowTilemapAndScheduleTransfer(&state->data[0x114]);
    ClearWindowTilemapAndScheduleTransfer(&state->data[0x124]);
    ClearWindowTilemapAndScheduleTransfer(&state->data[0x134]);
    ClearWindowTilemapAndScheduleTransfer(&state->data[0x144]);
    ClearFrameAndWindow2(&state->data[0x144], 1);
    ClearFrameAndWindow2(&state->data[0x44], 0);
    RemoveWindow(&state->data[0xe4]);
    RemoveWindow(&state->data[0xf4]);
    RemoveWindow(&state->data[0x104]);
    RemoveWindow(&state->data[0x114]);
    RemoveWindow(&state->data[0x124]);
    RemoveWindow(&state->data[0x134]);
    RemoveWindow(&state->data[0x144]);
}
