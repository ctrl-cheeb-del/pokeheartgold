...
s32 ov93_0225EA6C(void *p, s32 b) {
    ...
    return ov93_022630D8[U32AT(p, 0x270)] * n / 100;
}

void ov93_0225EA98(void *p) {
    if (S32AT(p, 0x244) >= 0x62) {
        S32AT(p, 0x248) += 1 << 0xc;
    } else if (S32AT(p, 0x244) >= 0x62) {
        S32AT(p, 0x248) += 1 << 0xc;
    } else {
        S32AT(p, 0x248) = 2 << 0xc;
    }
    if (S32AT(p, 0x248) > 0x11000) {
        S32AT(p, 0x248) = 0x11000;
    }
}
...
void ov93_0225EB38(void *p) {
    if (U32AT(p, 0x238) != 0) {
        U32AT(p, 0x238) = 0;
        S32AT(p, 0x23c) = -1;
        S32AT(p, 0x244) = S32AT(p, 0x240) - ov93_0225F9AC(p);
        if (S32AT(p, 0x244) < 0x19) {
            S32AT(p, 0x244) = 0;
        }
    }
}
