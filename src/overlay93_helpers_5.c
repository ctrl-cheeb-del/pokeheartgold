...
void ov93_022609E0(void *sys) {
    ...
    Ov93Slot *s;
    u32 t;
    for (i = 0, s = (Ov93Slot *)((u8 *)sys + 0x380c), t = 0; i < 5; i++, s++, t += 0x4000) {
        ov93_02260A58(sys, s, i, i);
        ManagedSprite_TickNFrames(s->sprite, t);
    }
}
...
void ov93_02260A14(void *sys) {
    ...
    u8 *p = (u8 *)sys;
    for (i = 0; i < 5; i++) {
        Sprite_DeleteAndFreeResources(PAT(p, 0x380c));
        p += 0xc;
    }
}
...
u32 ov93_02260A30(void *sys) {
    s32 i;
    Ov93Slot *s = (Ov93Slot *)((u8 *)sys + 0x380c);
    for (i = 0; i < 5; i++) {
        ov93_02260A8C(s);
        ov93_02260AD8(sys, s);
        s++;
...
u32 ov93_02260A58(void *sys, void *slot, u32 c, u32 d) {
    MI_CpuFill8(slot, 0, 0xc);
    U8AT(slot, 4) = (u8)ov93_02260B70((s32)d);
    U8AT(slot, 5) = 1;
    PAT(slot, 0) = ov93_02260984(sys, U8AT(slot, 4), c);
    U8AT(slot, 6) = (u8)c;
    U16AT(slot, 8) = (u16)d;
    return 1;
}
