...
u64 ov47_02259DA4(State *p, int i, u32 multiplier) {
    if ((u32)i >= 3) {
        GF_AssertFail();
    }
    return (u64)multiplier * *(u32 *)((u8 *)p + i * 4) / *(u64 *)((u8 *)p + 0xc);
}

void ov47_02259DCC(State *p, void *ctx) {
    int i;
    memset(p, 0, 0x14);
    for (i = 0; i < 0x14; i++) {
        void *obj = ov45_0222A578(ctx, i);
        if (obj != NULL) {
            void *value = ov45_0222AA54(obj);
            Ov47Temp temp;
            ov45_0222AB0C(obj, &temp);
            if (ov45_0222F6B0(0, value) == 1 && temp.category < 3) {
                (*(u32 *)((u8 *)p + temp.category * 4))++;
                (*(u64 *)((u8 *)p + 0xc))++;
            }
        }
    }
