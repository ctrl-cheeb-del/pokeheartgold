...
void ov28_0225DBFC(DowsingMchn *ptr) {
    u32 i;
    for (i = 0; i < 4; i++) {
        Sprite_Delete(ptr->sprites[i + 4]);
    }
    SpriteTransfer_DeleteCharTransferTask((SpriteResource *)ptr->resObjB[0]);
    SpriteTransfer_DeletePlttTransferTask((SpriteResource *)ptr->resObjB[1]);
}
