...
// Current-tree field names for the same verified layouts.
#define evalStep                            unk0
#define moveSlot                            unk1
#define move                                unk2
#define moveScore                           movePoints
#define calcTemp                            unk8
#define stateFlags                          unk10
#define aiBitShift                          unk11
#define moveDamageRolls                     unk18
#define trainerItems                        unk68
#define scriptStackPointer                  unk78
#define scriptStackSize                     unk98
#define trainerItemCounts                   unk99
#define attacker                            battlerIdAttacker
#define defender                            battlerIdTarget
#define usedItemType                        useItem
#define usedItemCondition                   unk9F
#define usedItem                            unkA0
#define selectedTarget                      unkA4
#define aiScriptTemp                        unk_2134
#define aiScriptCursor                      unk_2138
#define MoveEffectData                      UnkBattlemonSub
#define moveEffectData                      unk88
#define embargoTurns                        embargoFlag
#define aiSwitchedPartySlot                 unk_21A4
#define AI_FLAG_BASIC                       (1 << 0)
#define AI_FLAG_DOUBLES                     AI_DOUBLES
#define AI_FLAG_ROAMING_POKEMON             AI_29
#define BATTLER_SIDE_PLAYER                 0
#define BATTLE_TYPE_TRAINER_WITH_AI_PARTNER (BATTLE_TYPE_TRAINER | BATTLE_TYPE_DOUBLES | BATTLE_TYPE_MULTI | BATTLE_TYPE_AI)
#define MAX_TRAINER_ITEMS                   4
#define MAX_BATTLERS_PER_SIDE               2
#define MOVE_STATUS_IMMUNE                  (MOVE_STATUS_NO_EFFECT | MOVE_STATUS_WONDER_GUARD_IMMUNE | MOVE_STATUS_LEVITATE_IMMUNE | MOVE_STATUS_MAGNET_RISE_IMMUNE)
...
void ov10_0221BE20(BattleSystem *battleSystem, BattleContext *ctx, u8 battlerID, u8 initScore) {
    int i;
    u8 invalidMoves;
...
u8 ov10_0221BEF4(BattleSystem *battleSystem, u8 battlerID) {
    ...
    if (!(ctx->trainerAIData.stateFlags & AI_STATUS_FLAG_CONTINUE)) {
        ...
        ov10_0221BE20(battleSystem, ctx, ctx->trainerAIData.attacker, AI_INIT_SCORE_ALL_MOVES);
    }
...
void AICmd_IfSideCondition(BattleSystem *battleSystem, BattleContext *ctx) {
    ...
    int jump = AIScript_Read(ctx);
    u8 battler = AIScript_Battler(ctx, inBattler);
    u8 side = BattleSystem_GetFieldSide(battleSystem, battler);
...
void AICmd_CalcMaxEffectiveness(BattleSystem *battleSystem, BattleContext *ctx) {
    ...
    for (int i = 0; i < MAX_MON_MOVES; i++) {
        ...
        if (move) {
            damage = ov12_02251D28(battleSystem,
                ctx,
                move,
                moveType,
                ctx->trainerAIData.attacker,
...
static s32 TrainerAI_CalcDamage(BattleSystem *battleSystem, BattleContext *ctx, u16 move, u16 heldItem, u8 *ivs, int attacker, int ability, int embargoTurns, u8 variance) {
    ...
    defendingSide = BattleSystem_GetFieldSide(battleSystem, ctx->trainerAIData.defender);
    damage = 0;
    power = 0;
    type = 0;
...
static BOOL AI_CannotDamageWonderGuard(BattleSystem *battleSystem, BattleContext *ctx, int battler) {
    ...
    if (ctx->battleMons[battler ^ 1].ability == ABILITY_WONDER_GUARD) { // battler ^ 1 is BATTLER_OPP(battler) in pokeplatinum.
        ...
        for (i = 0; i < MAX_MON_MOVES; i++) {
            ...
            if (move) {
                effectiveness = 0;
                ov12_02251D28(battleSystem, ctx, move, moveType, battler, battler ^ 1, 0, &effectiveness);
...
static BOOL AI_OnlyIneffectiveMoves(BattleSystem *battleSystem, BattleContext *ctx, int battler) {
    ...
    for (i = 0; i < MAX_MON_MOVES; i++) {
        ...
        if (move && ctx->trainerAIData.moveData[move].power) {
            ...
            if (ctx->battleMons[defender1].hp) {
                ov12_02251D28(battleSystem, ctx, move, type, battler, defender1, 0, &effectiveness);
            }
            ...
            if (ctx->battleMons[defender2].hp) {
                ov12_02251D28(battleSystem, ctx, move, type, battler, defender2, 0, &effectiveness);
            }
    ...
    for (i = start; i < end; i++) {
        ...
        if (GetMonData(mon, MON_DATA_HP, NULL) != 0
            ...
            for (j = 0; j < MAX_MON_MOVES; j++) {
                move = GetMonData(mon, MON_DATA_MOVE1 + j, NULL);
                type = ov12_02258BB4(battleSystem, ctx, mon, move);
...
static BOOL AI_HasSuperEffectiveMove(BattleSystem *battleSystem, BattleContext *ctx, int battler, BOOL flag) {
    ...
    oppositeSlot = ov12_0223AB0C(battleSystem, battler) ^ 1;
    ...
    if ((ctx->switchInFlag & MaskOfFlagNo(defender)) == FALSE) {
        ...
        for (i = 0; i < MAX_MON_MOVES; i++) {
            ...
            if (move) {
                effectiveness = 0;
                ov12_02251D28(battleSystem, ctx, move, type, battler, defender, 0, &effectiveness);
...
int ov10_022205BC(BattleSystem *battleSystem, int battler) {
    // must declare C89-style to match
    int i;
    ...
    if ((battleType & BATTLE_TYPE_TRAINER) || BattleSystem_GetFieldSide(battleSystem, battler) == BATTLER_SIDE_PLAYER) {
        if (TrainerAI_ShouldSwitch(battleSystem, ctx, battler)) {
            ...
            if (ctx->aiSwitchedPartySlot[battler] == 6) {
                if ((i = ov12_02258800(battleSystem, battler)) == 6) {
                    battler1 = battler;
...
static BOOL TrainerAI_ShouldUseItem(BattleSystem *battleSystem, int battler) {
    ...
    if ((battleSystem->battleType & BATTLE_TYPE_TRAINER_WITH_AI_PARTNER) == BATTLE_TYPE_TRAINER_WITH_AI_PARTNER
        && ov12_0223AB0C(battleSystem, battler) == BATTLER_TYPE_PLAYER_SIDE_SLOT_2) {
        return result;
    }
