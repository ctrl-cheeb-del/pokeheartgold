...
void ov93_0225C6D8(void *p, void *args) {
    s32 i;
    s32 count;
    ...
    for (i = 0; i < 4; i++) {
        if (sub_02034818(i) != 0) {
            U8AT(p, 0x2c + count) = (u8)i;
            count++;
        } else {
            U8AT(p, 0x2c + i) = 0xff;
        }
    }
    U8AT(p, 0x30) = (u8)count;
    U8AT(p, 0x3c) = U8AT(args, 0x38);
    ov90_02258800(p, U8AT(args, 0x39), U32AT(args, 0x34), U8AT(args, 0x38), args);
}

u32 ov93_0225C730(void *p) {
    if (U8AT(p, 0x3d) == 0) {
        if (ov90_02258B54(p) == 1) {
            U8AT(p, 0x3d) = 1;
        }
    }
    if (U8AT(p, 0x3d) == 1) {
