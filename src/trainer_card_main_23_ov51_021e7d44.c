...
void ov51_021E7D44(TrainerCardMainState *state, int sprite, int sequence, int draw) {
    int flag = draw;
    u8 *sprites = &state->data[0x1f0];
    int offset = sprite * 4;
    Sprite_SetAnimCtrlSeq(*(void **)(sprites + offset), sequence);
    Sprite_ResetAnimCtrlState(*(void **)(sprites + offset));
    Sprite_SetDrawFlag(*(void **)(sprites + offset), flag);
}

void ov51_021E7D68(void) {
    ObjTransfer16 config = ov51_021E7FCC;
