...
void ov01_021E9BB8(FieldSystem *fieldSystem, u32 idx) {
    int se = GetDoorSE(fieldSystem, MapPropOneShotAnimationManager_GetAnimationMapPropModelID(fieldSystem->mapPropOneShotAnimationManager, idx), TRUE);
    MapPropOneShotAnimationManager_PlayAnimationWithSoundEffect(fieldSystem->mapPropOneShotAnimationManager, idx, FALSE, se);
}

void ov01_021E9BDC(FieldSystem *fieldSystem, u32 idx) {
    int se = GetDoorSE(fieldSystem, MapPropOneShotAnimationManager_GetAnimationMapPropModelID(fieldSystem->mapPropOneShotAnimationManager, idx), FALSE);
    MapPropOneShotAnimationManager_PlayAnimationWithSoundEffect(fieldSystem->mapPropOneShotAnimationManager, idx, TRUE, se);
}

void ov01_021E9C00(FieldSystem *fieldSystem, u8 idx) {
    u8 *env = Heap_AllocAtEnd(HEAP_ID_FIELD1, 1);
    *env = idx;
    TaskManager_Call(fieldSystem->taskman, ov01_021E9ABC, env);
}

void ov01_021E9C20(FieldSystem *fieldSystem, u32 idx) {
    MapPropOneShotAnimationManager_UnloadAnimation(fieldSystem->mapPropAnimationManager, fieldSystem->mapPropOneShotAnimationManager, idx);
}

u8 *ov01_021E9C30(void) {
    u8 *ptr = Heap_AllocAtEnd(HEAP_ID_FIELD1, 8);
    *ptr = 0;
    return ptr;
}
