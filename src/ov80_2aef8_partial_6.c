...
BOOL ov80_0222BC48(void *ctx, u16 value) {
    u8 *p = ctx;
    *(u16 *)(p + 0x424) = value;
    return sub_02037030(0x44, p + 0x424, 0x28) == 1;
}
...
BOOL ov80_0222BC94(void *ctx, u16 value) {
    u8 *p = ctx;
    *(u16 *)(p + 0x424) = value;
    return sub_02037030(0x45, p + 0x424, 0x28) == 1;
}
...
BOOL ov80_0222BCE0(void *ctx) {
    u8 *p = ctx;
    s32 count = BattleArcade_GetMonCount(p[0x10], 0);
    ...
    if (count > 0) {
        ...
        do {
            ...
            MI_CpuCopy8(mon, p + 0x474 + offset, size);
            i++;
            offset += size;
        } while (i < count);
    }
    return sub_02036FD8(0x46, p + 0x474, 0x200) == 1;
}
