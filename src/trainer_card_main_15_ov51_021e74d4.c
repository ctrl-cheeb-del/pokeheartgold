...
void ov51_021E74D4(void *windows, u8 first, u8 last) {
    ...
    for (i = first; i <= last; i++) {
        ClearWindowTilemapAndCopyToVram((u8 *)windows + i * 0x10);
    }
}

void ov51_021E74F4(void *window, int right, int gap, int y, void *string, int value, u8 digits, int mode) {
    int width;
    String16_FormatInteger(string, value, digits, mode, 1);
    width = FontID_String_GetWidth(0, string, 0);
    AddTextPrinterParameterizedWithColor(window, 0, string, right - (width + gap), y, 0xff, 0x10200, 0);
}

void ov51_021E7540(void *window, int right, int gap, int y, void *string) {
    int width = FontID_String_GetWidth(0, string, 0);
    AddTextPrinterParameterizedWithColor(window, 0, string, right - (width + gap), y, 0xff, 0x10200, 0);
}
