...
void ov108_021E838C(SafariAreaCustomizer *s) {
    GF_CreateVramTransferManager(0x20, s->heapId);
    s->spriteSystem = SpriteSystem_Alloc(s->heapId);
    SpriteSystem_Init(s->spriteSystem, &ov108_021EA9A0, &ov108_021EA98C, 4);
    thunk_ClearMainOAM(s->heapId);
    thunk_ClearSubOAM(s->heapId);
}

void ov108_021E83C8(SafariAreaCustomizer *s) {
    SpriteSystem_Free(s->spriteSystem);
    s->spriteSystem = NULL;
...
void ov108_021E83F0(SafariAreaCustomizer *s) {
    if (s->spriteSystem != NULL) {
        s->spriteMan = SpriteManager_New(s->spriteSystem);
        SpriteSystem_InitSprites(s->spriteSystem, s->spriteMan, 0x8C);
        sub_0200D2A4(s->spriteSystem, s->spriteMan, &ov108_021EA97C, 2, 0);
        ov108_021E853C(s->sub, SpriteManager_GetSpriteList(s->spriteMan));
    }
}

void ov108_021E844C(SafariAreaCustomizer *s) {
    if (s->spriteMan != NULL) {
        SpriteSystem_DestroySpriteManager(s->spriteSystem, s->spriteMan);
        s->spriteMan = NULL;
    }
}
...
void ov108_021E846C(SafariAreaCustomizer *s) {
    if (s->sub != NULL) {
        ov108_021E852C(s->sub);
    }
    if (s->spriteMan != NULL) {
        SpriteSystem_DrawSprites(s->spriteMan);
    }
}

void ov108_021E8490(SafariAreaCustomizer *s) {
    if (s->spriteSystem != NULL) {
        SpriteSystem_TransferOam(s->spriteSystem);
    }
}

Ov108Sub *ov108_021E84A4(u32 heapId, u16 a, u16 b, void *c) {
    Ov108Sub *p = Heap_Alloc(heapId, sizeof(Ov108Sub));

    MI_CpuFill8(p, 0, sizeof(Ov108Sub));
    p->heapId = heapId;
    p->unk0A = 4;
    p->unk0C = a;
    p->unk0E = b;
...
void ov108_021E84DC(Ov108Sub *s) {
    ov108_021E8718(s);
    MI_CpuFill8(s, 0, sizeof(Ov108Sub));
    Heap_Free(s);
}
