...
int ov73_021E83D4(void *p) {
    int r = YesNoPrompt_HandleInput(p);
    if (r) {
...
void *ov73_021E83F4(void *unused, u32 size, s32 align) {
    OSIntrMode f = OS_DisableInterrupts();
    void *p = NNS_FndAllocFromExpHeapEx(_021EA940[1], size, align);
    OS_RestoreInterrupts(f);
    return p;
}

void ov73_021E841C(void *unused, void *p) {
    if (p) {
        OSIntrMode f = OS_DisableInterrupts();
        NNS_FndFreeToExpHeap(_021EA940[1], p);
        OS_RestoreInterrupts(f);
    }
}

int ov73_021E8440(void *p) {
    return 3 - ov00_021EC9D4(p);
}
...
void ov73_021E8454(void *p) {
    ...
    ov73_021E8198(p);
    ov73_021E82D4(p);
    Main_SetVBlankIntrCB(ov73_021E8100, p);
}
