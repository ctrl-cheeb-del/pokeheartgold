...
int FrtCmd_128(void *c) {
    void *a = Frontier_GetLaunchArgs(APP(c));
    u8 *q = PTR(c, 0x1c);
    PTR(c, 0x1c) = q + 1;
    BufferRivalsName(PTR(FSYS(c), 0x44), *q, PTR(a, 8));
    return 0;
}

int FrtCmd_129(void *c) {
    void *a = Frontier_GetLaunchArgs(APP(c));
    u16 *out = FrontierScript_ReadVarPtr(c);
    *out = FrontierData_BattlePointAction(Save_FrontierData_Get(PTR(a, 8)), 0, 0);
    return 0;
}

int FrtCmd_130(void *c) {
    void *a = Frontier_GetLaunchArgs(APP(c));
    u16 v = FrontierScript_ReadVar(c);
    GameStats_Add(Save_GameStats_Get(PTR(a, 8)), 0x45, v);
    FrontierData_BattlePointAction(Save_FrontierData_Get(PTR(a, 8)), v, 5);
    return 0;
}

int FrtCmd_131(void *c) {
    void *a = Frontier_GetLaunchArgs(APP(c));
    u16 v = FrontierScript_ReadVar(c);
    GameStats_Add(Save_GameStats_Get(PTR(a, 8)), 0x46, v);
    FrontierData_BattlePointAction(Save_FrontierData_Get(PTR(a, 8)), v, 6);
    return 0;
}

int FrtCmd_053(void *c) {
    u16 v = FrontierScript_ReadVar(c);
    U16(c, 0x78) = v;
    sub_02037AC0((u8)v);
    FrontierScriptContext_Pause(c, ov80_0222D200);
    return 1;
}
...
BOOL ov80_0222D200(void *c) {
    ...
    return sub_02037B38((u8)U16(c, 0x78));
}
...
int FrtCmd_055(void *c) {
    sub_02058284(c);
    FrontierScriptContext_Pause(c, ov80_0222D244);
    return 1;
}
