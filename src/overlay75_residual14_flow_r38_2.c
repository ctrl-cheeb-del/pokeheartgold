...
int ov75_02248F18(Ov75Task *task) {
    ...
    switch (task->state) {
    case 0:
        ov75_022494CC(task, task->msgData, 0x15, 1, 0xF0F);
        task->state++;
        break;
    case 1:
        ov75_02246BCC(task->work, 3, 2);
        ov75_02246BE0(task->work, 0x18);
        task->next = 0x1E;
        break;
    }
    return 0;
...
int ov75_02248F7C(Ov75Task *task) {
    ...
    switch (task->state) {
    case 0:
        ov75_022494CC(task, task->msgData, 0x18, 1, 0xF0F);
        task->state++;
        break;
    case 1:
        ov75_02246C3C(task->work);
        ov75_02246C68(task->work);
        ov75_02246C8C(task->work);
        ov75_02247854(task, 0x19, 0);
        break;
    }
    return 0;
}
...
int ov75_02248FE8(Ov75Task *task) {
    switch (task->state) {
    case 0:
        ov75_022494CC(task, task->msgData, 0x20, 0, 0xF0F);
        ov75_0224785C(task);
        task->state++;
        break;
    case 1:
        if (SaveGameNormal(*(void **)((u8 *)task->work + 4)) == 2) {
            void *profile = Save_PlayerData_GetProfile(*(void **)((u8 *)task->work + 4));
            BufferPlayersName(task->format, 0, profile);
            ov75_022494CC(task, task->msgData, 0x21, 0, 0xF0F);
            PlaySE(0x61A);
            Save_ClearStatusFlags(*(void **)((u8 *)task->work + 4));
        } else {
            ov75_022494CC(task, task->msgData, 0x22, 0, 0xF0F);
        }
        ov75_02247878(task);
        task->state++;
        break;
    case 2:
        if (ov75_02249534(task->printer) == 0) {
            task->state++;
        }
        break;
    case 3:
        task->timer++;
        if ((s32)task->timer > 0x3C) {
            task->timer = 0;
            task->next = task->returnState;
        }
        break;
    }
    return 0;
...
int ov75_022490D8(Ov75Task *task) {
    ...
    switch (task->state) {
    case 0:
        ov75_022494CC(task, task->msgData, 0xF, 1, 0xF0F);
        task->state++;
        break;
    case 1:
        ov00_021EC8D8();
        ...
        if (*(u32 *)((u8 *)task->work + 0x118) == 1) {
            ov70_02237F2C();
            *(u32 *)((u8 *)task->work + 0x118) = 0;
        }
        task->state++;
        break;
    case 2:
        if (ov75_02249534(task->printer) == 0) {
            ov75_022494CC(task, task->msgData, 0x10, 1, 0xF0F);
            task->state++;
        }
        break;
    case 3:
...
int ov75_022491CC(Ov75Task *task) {
    s32 result = ov00_021E6A70(-task->unk18, task->unk1C);
    ov75_0224971C(task, result, -task->unk18);
    task->next = 0x1C;
    return 0;
}
