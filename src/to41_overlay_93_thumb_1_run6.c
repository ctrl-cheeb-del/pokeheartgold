...
void ov93_0225D468(void *p) {
    void *str;
    DrawFrameAndWindow1((u8 *)p + 0x70, 1, 0x350, 6);
    str = NewString_ReadMsgData(PAT(p, 0x80), 3);
    AddTextPrinterParameterizedWithColor((u8 *)p + 0x70, 0, str, 0, 0, 0, 0x1020f, 0);
    String_Delete(str);
    ScheduleWindowCopyToVram((u8 *)p + 0x70);
}

void ov93_0225D4B8(void *p) {
    FillBgTilemapRect(PAT(p, 0x2c), 4, 0x3ff, 1, 0x12, 0x1e, 6, 0x11);
    ScheduleBgTilemapBufferTransfer(PAT(p, 0x2c), 4);
}
