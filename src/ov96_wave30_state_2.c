...
BOOL ov96_021E839C(Work*w){void*save=Save_Pokeathlon_Get(**(void***)(w->d+0x1f8));u32 idx;u8*records;u16 value;if(*(u16*)(w->d+0xd2a)==0xffff){GF_AssertFail();return FALSE;}idx=PokeathlonCourse_GetField3D8_ForCurrentParticipant(w);if(*(u32*)(*(u8**)(w->d+0x1f8)+4)==1)records=PokeathlonSave_GetRecordsLink2(save),records+=0xa4*idx;else records=PokeathlonSave_GetRecordsSolo2(save),records+=0x2c*idx;value=*(u16*)records;if(value==0xffff)return TRUE;if(_0221A7D8[idx]==0){if(value<*(u16*)(w->d+0xd2a))return TRUE;}else if(value>*(u16*)(w->d+0xd2a))return TRUE;return FALSE;}
