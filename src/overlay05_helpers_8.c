...
void ov05_0221CFF0(Overlay05State *state) {
    if (*(void **)&state->data[0xb94] != NULL) {
        RemoveWindow(&state->data[0xb88]);
    }
    if (*(void **)&state->data[0xba4] != NULL) {
        RemoveWindow(&state->data[0xb98]);
    }
}

void ov05_0221D020(Overlay05State *state) {
    if (*(void **)&state->data[0xbb4] != NULL) {
        String_Delete(*(void **)&state->data[0xbb4]);
    }
    if (*(void **)&state->data[0xbb0] != NULL) {
        MessageFormat_Delete(*(void **)&state->data[0xbb0]);
    }
    if (*(void **)&state->data[0xbac] != NULL) {
        DestroyMsgData(*(void **)&state->data[0xbac]);
    }
}

void ov05_0221D054(Overlay05State *state) {
    GfGfx_EngineATogglePlanes(0x1f, 0);
    FreeBgTilemapBuffer(*(void **)&state->data[0xc], 3);
    FreeBgTilemapBuffer(*(void **)&state->data[0xc], 2);
    FreeBgTilemapBuffer(*(void **)&state->data[0xc], 1);
    if (*(void **)&state->data[0xba8] == NULL) {
        FreeBgTilemapBuffer(*(void **)&state->data[0xc], 0);
    }
    Heap_Free(*(void **)&state->data[0xc]);
}
