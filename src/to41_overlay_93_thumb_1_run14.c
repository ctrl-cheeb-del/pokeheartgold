...
void ov93_0225E008(void *p, s32 a, s32 b) {
    u8 *base = (u8 *)p + 0xd8;
    u8 *q;
    sub_020181E0(base + 0xcc + a * 0x240 + b * 0x90, base + 0x144 + a * 0x240 + b * 0x90);
    q = base + a * 0x240;
    q += b * 0x90;
    U32AT(q, 0xc8) = 0;
}
...
void ov93_0225E03C(void *p) {
    u8 *workRow = (u8 *)p + 0xd8;
    u8 *objectRow;
    s32 i;
    s32 j;

    i = 0;
    objectRow = workRow + 0x144;
    for (; i < 8; i++) {
        u8 *work = workRow;
        u8 *object = objectRow;
        for (j = 0; j < 4; j++) {
            if (*(u32 *)(work + 0xc8) == 1 && sub_0201815C(object, 0x1000) == 1) {
                ov93_0225E008(p, i, j);
            }
            work += 0x90;
            object += 0x90;
