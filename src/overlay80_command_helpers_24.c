...
int FrtCmd_060(void *c) {
    FrontierScript_ReadVarPtr(c);
    return 1;
}

int FrtCmd_061(void *c) {
    u16 n = FrontierScriptContext_ReadHalfWord(c);
    u16 v = FrontierScript_ReadVar(c);
    void *a = Frontier_GetLaunchArgs(APP(c));
    *Save_VarsFlags_GetVarAddr(Save_VarsFlags_Get(PTR(a, 8)), n) = v;
    return 0;
}

int FrtCmd_062(void *c) {
    u16 n = FrontierScriptContext_ReadHalfWord(c);
    u16 *out = FrontierScript_ReadVarPtr(c);
    void *a = Frontier_GetLaunchArgs(APP(c));
    *out = *Save_VarsFlags_GetVarAddr(Save_VarsFlags_Get(PTR(a, 8)), n);
    return 0;
}
