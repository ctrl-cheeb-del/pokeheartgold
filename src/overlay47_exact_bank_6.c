...
void ov47_02259968(State *p) {
    int i;
    ov47_02259D24((State *)((u8 *)p + 0x88));
    ov47_02259D24((State *)((u8 *)p + 0xA8));
    Heap_Free(*(void **)((u8 *)p + 0x80));
    *(void **)((u8 *)p + 0x80) = NULL;
    for (i = 0; i < 6; i++, p = (State *)((u8 *)p + 0x10)) {
        RemoveWindow(p);
    }
}

void ov47_0225999C(void *window, State *messages, int msg, u32 color) {
    void *str;
    u32 width;
    FillWindowPixelBuffer(window, 0);
    str = ov47_02258CEC(messages, 0, msg);
    width = FontID_String_GetWidth(1, str, 0);
    AddTextPrinterParameterizedWithColor(window, 1, str, (0x90 - width) / 2, 4, 0xff, color, NULL);
