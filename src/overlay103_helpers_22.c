...
void ov103_021EE824(void *a0) {
    void *s;
    ov103_021EE784(a0, 0);
    s = NewString_ReadMsgData(PAT(PAT(a0, W_SUB), S_MSGDATA), 9);
    StringExpandPlaceholders(PAT(PAT(a0, W_SUB), S_MSGFMT), PAT(PAT(a0, W_SUB), S_STRING), s);
    String_Delete(s);
    ov103_021EE7DC(a0);
}

void ov103_021EE860(void *a0) {
    DrawFrameAndWindow2(WIN(PAT(a0, W_SUB), 24), 1, 0x3E2, 0xd);
    ScheduleWindowCopyToVram(WIN(PAT(a0, W_SUB), 24));
}

void ov103_021EE888(void *a0) {
    ClearFrameAndWindow2(WIN(PAT(a0, W_SUB), 24), 1);
    ScheduleBgTilemapBufferTransfer(PAT(PAT(a0, W_SUB), S_BGCFG), 0);
}
