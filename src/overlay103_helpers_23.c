...
s32 ov103_021EEA24(void *a0) {
    if (U16AT(PAT(a0, W_SUB), S_MAXPAGE) == 0) {
        return -1;
    }
    return TouchscreenHitbox_FindRectAtTouchNew(ov103_021EEFBC);
}

void ov103_021EEA48(void *a0) {
    u32 col;
    if (U16AT(a0, W_PAGE) > U16AT(PAT(a0, W_SUB), S_MAXPAGE)) {
        U16AT(a0, W_PAGE) = U16AT(PAT(a0, W_SUB), S_MAXPAGE);
        col = 0;
    } else {
        col = U8AT(a0, W_COL);
    }
    PAT(PAT(a0, W_SUB), S_GRID) = GridInputHandler_Create(ov103_021EEFD8, ov103_021EF008, ov103_021EEFC8, a0, 1, (u8)col, 0x9d);
    ov103_021EEAC8(a0, col);
}

void ov103_021EEAA0(void *a0) {
    GridInputHandler_Free(PAT(PAT(a0, W_SUB), S_GRID));
}
...
void ov103_021EEAC8(void *a0, u32 a1) {
    u8 *box = (u8 *)GridInputHandler_GetDpadBox(PAT(PAT(a0, W_SUB), S_GRID), a1);
    ov103_021EE0F8(PAT(a0, W_SUB), 2, box[0], box[1]);
    if (a1 == 0xa) {
        ov103_021EE0AC(PAT(a0, W_SUB), 2, 4);
    } else {
        ov103_021EE0AC(PAT(a0, W_SUB), 2, 5);
    }
}

u32 ov103_021EEB04(void) {
    if (TouchscreenHitbox_FindRectAtTouchNew(ov103_021EEFB4) != -1) {
        return 1;
    }
    if ((3 & NEWKEYS) != 0) {
        return 1;
