...
void ov103_021ED23C(void *a0) {
    void *narc = NARC_New(0x4f, 0x9d);
    u32 type = Mail_GetType(PAT((u8 *)PAT(a0, W_SUB) + U8AT(a0, W_SLOT) * 4, S_MAIL));
    GfGfxLoader_LoadCharDataFromOpenNarc(narc, type + 0xc, PAT(PAT(a0, W_SUB), S_BGCFG), 5, 0, 0, 0, 0x9d);
    GfGfxLoader_GXLoadPalFromOpenNarc(narc, type, 4, 0, 0x40, 0x9d);
    GfGfxLoader_LoadScrnDataFromOpenNarc(narc, type + 0x18, PAT(PAT(a0, W_SUB), S_BGCFG), 5, 0, 0, 0, 0x9d);
    NARC_Delete(narc);
}

u32 ov103_021ED2B8(void *a0) {
    if ((s32)Party_GetCount(SaveArray_Party_Get(PAT(PAT(a0, W_ARGS), 0))) > 0) {
        return 4;
    }
    return 2;
}
...
BOOL ov103_021ED2D4(void *a0, u32 *a1) {
    *a1 = ov103_021EECA8[*a1](a0);
    if (*a1 == 0x19) {
        return 0;
    }
    if (PAT(a0, W_SUB) != NULL) {
        sub_02019934(PAT(PAT(a0, W_SUB), S_WINSET));
        ov103_021EDF68(PAT(a0, W_SUB));
        SpriteSystem_DrawSprites(PAT(PAT(a0, W_SUB), S_SPRMAN));
    }
    return 1;
}
...
u32 ov103_021ED314(void *a0) {
    ...
    *(u16 *)0x04001050 = 0;
    sub_020210BC();
    sub_02021148(4);
    *(u16 *)0x04000304 &= 0xFFFF7FFF;
    Heap_Create(3, 0x9d, 3 << 0x11);
    sub = Heap_Alloc(0x9d, 0x2f << 4);
    PAT(a0, W_SUB) = sub;
    MI_CpuFill8(sub, 0, 0x2f << 4);
    ov103_021ED00C(a0);
    ov103_021EC9D8();
    ov103_021EC9E8(a0);
    ov103_021ECC1C(a0);
    ...
    ov103_021ECE80(a0);
    ov103_021EEA48(a0);
    ov103_021ECE18(PAT(a0, W_SUB));
    ov103_021EE550(a0);
    ov103_021EE644(a0);
    ov103_021ED0C0(a0);
    ov103_021ECEEC(a0);
    Main_SetVBlankIntrCB(ov103_021EC9B4, a0);
    return U32AT(a0, W_STATE);
}

u32 ov103_021ED3E8(void *a0) {
    Main_SetVBlankIntrCB(NULL, NULL);
    ov103_021ECF48(a0);
    ov103_021ECE54(PAT(a0, W_SUB));
    ov103_021EEAA0(a0);
    ov103_021ECE94(a0);
    ov103_021EDF50(a0);
    ov103_021ECE0C(a0);
...
u32 ov103_021ED460(void *a0) {
    if (IsPaletteFadeFinished() == 1) {
        return U32AT(PAT(a0, W_SUB), S_NEXT);
    }
    return 2;
}

u32 ov103_021ED47C(void *a0) {
    if (TextPrinterCheckActive(U8AT(PAT(a0, W_SUB), 0x8d << 2)) == 0) {
        if (U8AT(PAT(a0, W_SUB), S_FLAG235) == 0) {
            return U32AT(a0, W_STATE);
        }
        if (System_GetTouchNew() == 1) {
            PlaySE(0x5DC);
            return U32AT(a0, W_STATE);
        }
        if ((3 & NEWKEYS) != 0) {
