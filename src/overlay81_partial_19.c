...
void ov81_02242F10(void **p, int x, int y) {
    VecFx32 v;
    v.x = x << 12;
    v.y = (y << 12) + (2 << 20);
    Sprite_SetMatrix(p[3], &v);
}

void *ov81_02242F30(void **p) {
    return Sprite_GetMatrixPtr(p[3]);
}

void ov81_02242F3C(u16 *p, u16 v) {
    *p = v;
}

u8 ov81_02242F40(u16 *p) {
    return *p;
}

void ov81_02242F48(u16 *p) {
    ov81_02242F3C(p, 1);
}

void ov81_02242F54(u16 *p) {
    ov81_02242F3C(p, 0);
}
