...
void ov48_02259824(void *p, void *q, u32 heapId) {
    ...
    U32AT(p, 0x14) = 0x1000;
    U32AT(p, 0x18) = 0x69 << 6;
    U32AT(p, 0x1C) = 0x1F << 10;
    U32AT(p, 0x20) = 0;
    sub_02018030(ADD(p, 0x9C), PTRAT(q, 0x144), 3, heapId);
    sub_020181B0(ADD(p, 0x24), ADD(p, 0x9C));
}

void ov48_02259868(void *p) {
    sub_02018068(ADD(p, 0x9C));
}

void ov48_02259874(void *p) {
    u32 buf[9];
    ov48_022598BC(p, buf);
    sub_020182A8(ADD(p, 0x24), U32AT(p, 0), U32AT(p, 4), U32AT(p, 8));
    sub_020182C4(ADD(p, 0x24), U32AT(p, 0xC), U32AT(p, 0x10), U32AT(p, 0x14));
    sub_02018288(ADD(p, 0x24), buf);
}
...
void ov48_022598BC(void *p, void *a) {
    ov48_02258BF4(a, ADD(p, 0x18));
}

void ov48_022598CC(void *p, VecFx32 *dst) {
    *dst = *(VecFx32 *)ADD(p, 0x18);
}

void ov48_022598DC(void *p, VecFx32 *src) {
    *(VecFx32 *)ADD(p, 0x18) = *src;
}
