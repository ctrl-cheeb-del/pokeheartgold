...
BOOL ov12_0223A218(OverlayManager *man) {
    Ov12MainData *data = OverlayManager_GetData(man);
    int i;
    if (data->heapGroup->enabled) {
        for (i = 0; i < 4; i++) {
            if (data->heapGroup->allocations[i] != NULL) {
                Heap_Free(data->heapGroup->allocations[i]);
            }
        }
        Heap_Free(data->heapGroup);
        return TRUE;
    }
    return FALSE;
}
