...
void ov103_021EE374(void *sub) {
    ov103_021EE0F8(sub, 2, 0xe0, 0xb0);
    ov103_021EE0AC(sub, 2, 4);
}

void ov103_021EE390(void *a0) {
    u32 i;
    const u8 *tmpl;
    u32 off;
    FontID_Alloc(4, 0x9d);
    ...
    off = 0;
    do {
        AddWindow(PAT(PAT(a0, W_SUB), S_BGCFG), (void *)((u8 *)PAT(a0, W_SUB) + 0x48 + off), tmpl);
        i++;
        tmpl += 8;
        off += 0x10;
    } while (i < 0x1e);
...
void ov103_021EE3C0(void *a0) {
    u32 i = 0;
    u32 off = 0;
    do {
        RemoveWindow((void *)((u8 *)PAT(a0, W_SUB) + 0x48 + off));
        i++;
        off += 0x10;
    } while (i < 0x1e);
    FontID_Release(4);
...
void ov103_021EE3E4(void *window, void *str, u32 x, u32 y, u32 fontId, u32 color, u32 align) {
    if (align == 1) {
...
void ov103_021EE430(void *sub, void *msgData, u32 winIdx, u32 msgId, u32 x, u32 y, u32 fontId, u32 color, u32 align) {
    void *str = NewString_ReadMsgData(msgData, msgId);
    ov103_021EE3E4(WIN(sub, winIdx), str, x, y, fontId, color, align);
    String_Delete(str);
}
