...
void ov82_0223F8E4(Ov82Work *work) {
    ov82_0223F90C(work);
    ov82_0223F5E0(work->bgConfig, work->unk00D, 0);
    ScheduleBgTilemapBufferTransfer(work->bgConfig, 3);
    ov82_0223FCFC(work->spr204, 1);
}

void ov82_0223F90C(Ov82Work *work) {
    ov82_0223FCB0(work->spr208, TRUE);
    ClearFrameAndWindow2(&work->windows[0], 0);
    ov82_0223EFCC(work, &work->windows[1], 0, 0, 1, 2, 0, 0);
}

void ov82_0223F948(s32 a) {
    G2x_SetBlendBrightness_((volatile u16 *)0x04000050, 0x1e, a);
}
