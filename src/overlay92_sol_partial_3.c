...
int ov92_0225D868(int mask) {
    BeginNormalPaletteFade(0, mask, mask, 0, 6, 1, 0x71);
    return 1;
}
...
void ov92_0225D894(void *p) {
    volatile u32 *irqBase = (u32 *)0x027e0000;
    GF_RunVramTransferTasks();
    SpriteSystem_TransferOam();
    PaletteData_PushTransparentBuffers(PAT(p, 0x5c));
    DoScheduledBgGpuUpdates(PAT(p, 0x58));
    irqBase[0x3ff8 / 4] |= 1;
}
