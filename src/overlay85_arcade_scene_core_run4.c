...
/* ---- run 4: 4 functions, source order == reference address order ---- */
BOOL ov85_021E87F0(void *party);
BOOL ov85_021E8834(ArcWork *work);
u32 ov85_021E8878(ArcWork *work);
u32 ov85_021E8898(ArcWork *work);
...
BOOL ov85_021E87F0(void *party) {
    ...
    for (i = 0; i < count; i++) {
        ...
        if (GetMonData(mon, MON_DATA_CHECKSUM_FAILED, NULL) == 0) {
            continue;
        }
        return TRUE;
    }
...
BOOL ov85_021E8834(ArcWork *work) {
    int i;
    if (ov85_021E87F0(work->unk_024) == 1) {
        return TRUE;
    }
    for (i = 0; i < 5; i++) {
        if (ov85_021E86B0(work, i)) {
            if (ov85_021E87F0(sub_02097018(work->unk_0D0, i)) == 1) {
                return TRUE;
            }
        }
    }
