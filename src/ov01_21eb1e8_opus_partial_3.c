...
void ov01_021EB56C(void *a0, WeatherSys *sys) {
    SpriteList_RenderAndAnimateSprites(sys->unkF4);
}

void ov01_021EB578(void *a0, int a1, int a2) {
    void *hdr = GF2DGfxResHeader_GetByIndex(a0, a1);
    void *data = GfGfxLoader_LoadFromNarc(0x3F, a2, 0, 4, 1);
    GF2DGfxResHeader_Init(data, hdr, 4);
    Heap_Free(data);
}
...
void ov01_021EB5A4(WeatherGfxMgr *m) {
    int i;
    void **p;
    for (i = 0, p = m->objMan; i < 4; i++, p++) {
        GF2DGfxResHeader_Reset(GF2DGfxResHeader_GetByIndex(m->resHdr, i));
        Destroy2DGfxResObjMan(p[0]);
    }
    Heap_Free(m->resHdr);
    m->resHdr = NULL;
    SpriteList_Delete(m->spriteList);
    m->spriteList = NULL;
    SysTask_Destroy(m->task);
    m->task = NULL;
}
