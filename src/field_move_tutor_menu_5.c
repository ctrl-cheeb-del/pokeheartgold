...
void ov01_021EE568(MoveTutorMenu *menu) {
    ...
    DestroyListMenu(menu->listMenu, NULL, NULL);
    sub_0200E5D4(menu->listTemplate.window, 0);
    RemoveWindow(&menu->window);
    for (i = 0; i < 28; i++) {
...
void ov01_021EE5D0(MoveTutorMenu *menu, u32 msgId, u32 a2) {
    String *tmp = String_New(90, HEAP_ID_FIELD1);
    String *dest = String_New(90, HEAP_ID_FIELD1);
    ...
    AddTextPrinterParameterized(menu->printWindow, 1, dest, 0, 0, a2, 0);
    String_Delete(tmp);
    String_Delete(dest);
}

void ov01_021EE634(MoveTutorMenu *menu) {
    ListMenuGetCurrentItemArrayId(menu->listMenu, &menu->cursorPos);
    if (menu->msgIds[menu->cursorPos] != 0xFF) {
        ov01_021EE5D0(menu, menu->msgIds[menu->cursorPos], 0);
    }
}
