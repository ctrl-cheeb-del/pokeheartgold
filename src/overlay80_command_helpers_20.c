...
int FrtCmd_116(void *c) {
    Save_WipeExtraChunks(PTR(Frontier_GetLaunchArgs(APP(c)), 8));
    return 0;
}

int FrtCmd_117(void *c) {
    void *a = Frontier_GetLaunchArgs(APP(c));
    u16 *out = FrontierScript_ReadVarPtr(c);
    *out = Save_CheckExtraChunksExist(PTR(a, 8));
    return 0;
}

int FrtCmd_119(void *c) {
    void *s = FSYS(c);
    PTR(s, 0x7c) = WaitingIcon_New((u8 *)s + 0x64, 0x3e2);
    return 0;
}

int FrtCmd_120(void *c) {
    sub_0200F450(PTR(FSYS(c), 0x7c));
    return 0;
}
