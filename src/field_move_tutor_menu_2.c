...
void ov01_021EDF00(MoveTutorMenu *menu) {
    int i;

    for (i = 0; i < 28; i++) {
        String_Delete(menu->strbufs[i]);
    }
    if (menu->ownMsgData == 1) {
        DestroyMsgData(menu->msgData);
    }
    Heap_Free(menu);
}

void ov01_021EDF38(MoveTutorMenu *menu) {
    PlaySE(SEQ_SE_DP_SELECT);
    Delete2dMenu(menu->menu2dPtr, NULL);
    sub_0200E5D4(menu->menu2d.window, 0);
    RemoveWindow(menu->menu2d.window);
    SysTask_Destroy(menu->task);
    ov01_021EDF00(menu);
}

MoveTutorMenu *ov01_021EDF78(FieldSystem *fieldSystem, u8 x, u8 y, u8 initCursorPos, u8 cancellable, u16 *ret_p, MessageFormat *msgFmt, Window *window, MsgData *msgData) {
    return ov01_021EDC28(fieldSystem, x, y, initCursorPos, cancellable, ret_p, msgFmt, window, msgData);
}

void MoveTutorMenu_SetListItem(MoveTutorMenu *menu, int msgId, int a2, int value) {
    MoveTutorMenu_SetListItem_Internal(menu, msgId, a2, value);
}
