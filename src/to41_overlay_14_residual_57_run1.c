...
void ov14_021F49E0(void *p) {
    ...
    off = U8_AT(p, 0x25) / 6 * 6;
    slot = 0xf;
    i = 0;
    off = off << 10;
    for (; i < 6; i++) {
        ov14_021F2C1C(PTR_AT(p, 0x34), slot, (u8 *)PTR_AT(p, 0x34) + 0x40c8 + off, 0x400);
        slot++;
        off += 0x400;
    }
}
...
void ov14_021F4A20(void *p, u32 slot) {
    if (U8_AT(p, 0x25) / 6 == slot / 6) {
        ov14_021F2C1C(PTR_AT(p, 0x34), slot % 6 + 0xf, (u8 *)PTR_AT(p, 0x34) + 0x40c8 + slot * 0x400, 0x400);
    }
}
