...
void ov51_021E6B88(TrainerCardMainState *state) {
    int pos = -state->data[0x343f];
    BgSetPosTextAndCommit(*(void **)&state->data[0], 5, 0, pos);
    BgSetPosTextAndCommit(*(void **)&state->data[0], 5, 3, pos);
    BgSetPosTextAndCommit(*(void **)&state->data[0], 3, 0, pos);
    BgSetPosTextAndCommit(*(void **)&state->data[0], 3, 3, pos);
    if (((u32)state->data[0x343a] << 29) >> 31) {
        ov51_021E69EC(state);
    }
    DoScheduledBgGpuUpdates(*(void **)&state->data[0]);
    GF_RunVramTransferTasks();
    OamManager_ApplyAndResetBuffers();
    *(vu32 *)((u8 *)OS_IRQTable + 0x3ff8) |= 1;
}
