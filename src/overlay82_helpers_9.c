...
u16 ov82_0223F558(Ov82Work *work) {
    return (u16)((work->unk00D % 4) * 0x40 + 0x20);
}

u16 ov82_0223F570(Ov82Work *work) {
    return (u16)((work->unk00D / 4) * 0x24 + 0x10);
}

void ov82_0223F580(Ov82Work *work, BgConfig *bgConfig) {
    int i;
    if (ov82_0223F6E4(work) == 1) {
        for (i = 0; i < 0x11; i++) {
            ov82_0223F5E0(bgConfig, (u8)i, 3);
        }
    } else {
        for (i = 0; i < 0x11; i++) {
            if (sub_02030BD0((u8)i, work->unk218) >= 0xa) {
                ov82_0223F5E0(bgConfig, (u8)i, 3);
            }
        }
        ov82_0223F5E0(bgConfig, 0x13, 3);
    }
