...
/* ---- run 3: 6 functions, source order == reference address order ---- */
void ov85_021E86AC(ArcWork *work, u32 data);
BOOL ov85_021E86B0(ArcWork *work, u32 a1);
void ov85_021E86CC(ArcWork *work, int a1);
int ov85_021E8720(ArcWork *work);
...
BOOL ov85_021E86B0(ArcWork *work, u32 a1) {
    if (a1 != 0) {
        if ((((ArcSubD0 *)work->unk_0D0)->unk_42 & (1 << a1)) == 0) {
            return FALSE;
        }
    }
    return TRUE;
...
void ov85_021E86CC(ArcWork *work, int a1) {
    void *owner = work->unk_024;
    void *party = sub_02097018(work->unk_0D0, a1);
    int idx = (&work->unk_044)[a1];
    Pokemon *dest = Party_GetMonByIndex(owner, ((ArcSubCC *)work->unk_0CC)->unk_04);
    Pokemon *mon = Party_GetMonByIndex(party, idx);
    MonSetTrainerMemo(mon, (PlayerProfile *)sub_02034818(sub_0203769C()), 5, 0, (enum HeapID)0xB);
    CopyPokemonToPokemon(mon, dest);
}
