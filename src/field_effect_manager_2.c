...
void ov01_021F17BC(FieldEffectManager *manager, FieldEffectGfx *gfx, u32 count) {
    FieldEffectRecord *records = ov01_021F141C(manager, count * sizeof(FieldEffectRecord), 0);
    FieldEffectSlot *slots;
    gfx->records = records;
    slots = ov01_021F141C(manager, count * sizeof(FieldEffectSlot), 0);
    gfx->slots = slots;
    do {
        slots->id = 0xf;
        slots->record = records;
        slots++;
        records++;
    } while (--count != 0);
}
...
void ov01_021F17F0(FieldEffectGfx *gfx) {
    ov01_021F1448(gfx->slots);
    ov01_021F1448(gfx->records);
}
