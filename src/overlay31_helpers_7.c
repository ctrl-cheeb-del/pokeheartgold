...
void ov31_0225E0E4(Overlay31State *state, int screen) {
    void *screenData;
    void *allocated = GfGfxLoader_GetScrnData(0x3c, 0x12, 0, &screenData, 8);
    BG_LoadScreenTilemapData(*(void **)&state->data[4], 5, (u8 *)screenData + 0xc, *(u32 *)((u8 *)screenData + 8));
    ov31_0225E060(*(void **)&state->data[4], 5, screen);
    ScheduleBgTilemapBufferTransfer(*(void **)&state->data[4], 5);
    Heap_Free(allocated);
}

int ov31_0225E12C(u8 *args, int index, int value) {
    if (args[0x283] == 3) {
        return PokeathlonSave_GetUnkB7C_AtIndex(*(void **)&args[0x254], index) == 0;
    }
    if (args[0x283] == 4) {
        return PokeathlonSave_GetUnkB78_AtIndex(*(void **)&args[0x254], index + ((value - 0x1f9) / 6) * 6) == 0;
    }
    return 1;
}
