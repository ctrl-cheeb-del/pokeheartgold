...
void ov103_021EE028(void *sub) {
    SpriteSystem_FreeResourcesAndManager(PAT(sub, S_SPRSYS), PAT(sub, S_SPRMAN));
    SpriteSystem_Free(PAT(sub, S_SPRSYS));
}

void *ov103_021EE048(void *sub, const void *tmpl) {
    if (U32AT(tmpl, 0x10) == 1) {
        return SpriteSystem_NewSprite(PAT(sub, S_SPRSYS), PAT(sub, S_SPRMAN), tmpl);
    }
    return SpriteSystem_NewSpriteWithYOffset(PAT(sub, S_SPRSYS), PAT(sub, S_SPRMAN), tmpl, 2 << 0x14);
}
