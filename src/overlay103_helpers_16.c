...
void ov103_021EDF50(void *a0) {
    ov103_021EE094(PAT(a0, W_SUB));
    ov103_021EE028(PAT(a0, W_SUB));
    ov103_021EE150(PAT(a0, W_SUB));
}

void ov103_021EDF68(void *sub) {
    u32 i = 0;
    u8 *p = (u8 *)sub;
    do {
        if (PAT(p, S_SPRITES) != NULL) {
            ManagedSprite_TickFrame(PAT(p, S_SPRITES));
        }
        i++;
        p += 4;
    } while (i < 7);
