...
void ov103_021ECE0C(void *a0) {
    sub_020195C0(PAT(PAT(a0, W_SUB), S_WINSET));
}

void ov103_021ECE18(void *sub) {
    PAT(sub, S_MSGDATA) = NewMsgDataFromNarc(0, 0x1b, 0xe8, 0x9d);
    PAT(sub, S_MSGFMT) = MessageFormat_New(0x9d);
    PAT(sub, S_STRING) = String_New(0x400, 0x9d);
    TextFlags_SetCanTouchSpeedUpPrint(1);
}

void ov103_021ECE54(void *sub) {
    TextFlags_SetCanTouchSpeedUpPrint(0);
    String_Delete(PAT(sub, S_STRING));
    MessageFormat_Delete(PAT(sub, S_MSGFMT));
    DestroyMsgData(PAT(sub, S_MSGDATA));
}

void ov103_021ECE80(void *a0) {
    void *p = YesNoPrompt_Create(0x9d);
    PAT(PAT(a0, W_SUB), S_YESNO) = p;
}

void ov103_021ECE94(void *a0) {
    YesNoPrompt_Destroy(PAT(PAT(a0, W_SUB), S_YESNO));
}

void ov103_021ECEA4(void *a0) {
    struct Ov103YesNoTemplate t;
    t.bgConfig = PAT(PAT(a0, W_SUB), S_BGCFG);
    t.unk4 = 0;
    t.msgId = 0x3A6;
    t.unkC = 0xa;
    t.x = 0x19;
    ...
    t.palB = 0;
    t.unk13 = 0;
    YesNoPrompt_InitFromTemplate(PAT(PAT(a0, W_SUB), S_YESNO), &t);
}

void ov103_021ECEEC(void *a0) {
    void *spawner = TouchscreenListMenuSpawner_Create(0x9d, 0);
    u32 n;
    u16 i;
    PAT(PAT(a0, W_SUB), 9 << 6) = spawner;
    n = ov103_021ED2B8(a0);
    PAT(PAT(a0, W_SUB), S_LISTITEMS) = ListMenuItems_New(n, 0x9d);
    i = 0;
    do {
        if (n != 2 || (i != 2 && i != 1)) {
            ListMenuItems_AppendFromMsgData(PAT(PAT(a0, W_SUB), S_LISTITEMS),
                PAT(PAT(a0, W_SUB), S_MSGDATA),
                i + 5,
                i);
        }
        i++;
...
void ov103_021ECF48(void *a0) {
    ListMenuItems_Delete(PAT(PAT(a0, W_SUB), S_LISTITEMS));
    TouchscreenListMenuSpawner_Destroy(PAT(PAT(a0, W_SUB), 9 << 6));
}
