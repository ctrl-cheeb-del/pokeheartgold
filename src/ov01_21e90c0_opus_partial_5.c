...
BOOL ov01_021E9EEC(TaskManager *taskman) {
    ...
    Struct021E9F78 *env = TaskManager_GetEnvironment(taskman);
    switch (env->state) {
    case 0:
        MapObject_SetVisible(PlayerAvatar_GetMapObject(fieldSystem->playerAvatar), TRUE);
        env->active = 0;
        env->step = 0;
        PlaySE(SEQ_SE_DP_KAIDAN2);
        BeginNormalPaletteFade(FADE_BOTH_SCREENS, FADE_TYPE_BRIGHTNESS_OUT, FADE_TYPE_BRIGHTNESS_OUT, 0x7FFF, 6, 1, HEAP_ID_FIELD2);
        env->active = 1;
        env->state++;
        break;
    case 1:
    ...
    if (env->active != 0) {
        ov01_021E95CC(fieldSystem->camera, &env->step);
    }
    return FALSE;
}
...
BOOL ov01_021E9F78(TaskManager *taskman) {
    ...
    case 0:
        ...
        env->step = 0;
        PlaySE(SEQ_SE_DP_KAIDAN2);
        NewFieldFadeEnvironment(taskman, FADE_BOTH_SCREENS, (enum FadeType)16, FADE_TYPE_BRIGHTNESS_OUT, 0, 6, 1, HEAP_ID_FIELD2);
        env->active = 1;
        env->state++;
        break;
    case 1:
