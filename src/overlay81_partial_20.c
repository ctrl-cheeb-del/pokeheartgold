...
void *ov81_02242F8C(void **p) {
    return p[1];
}

void *ov81_02242F90(void **p) {
    return p[2];
}

void ov81_02242F94(void **p, int n) {
    Sprite_SetAnimSpeed(p[3], 1 << 12);
    Sprite_SetAnimCtrlSeq(p[3], n);
}

void ov81_02242FB0(void **p, int v) {
    Sprite_SetPaletteOverride(p[3], v);
}

void ov81_02242FBC(void **p, int v) {
    Sprite_SetOamMode(p[3], v);
}

void *ov81_02242FC8(void **p) {
    Sprite_Delete(p[3]);
    Heap_Free(p);
    return NULL;
}
