...
void ov90_022591D4(void **p) {
    String_Delete(p[3]);
    String_Delete(p[2]);
    MessageFormat_Delete(p[1]);
    DestroyMsgData(p[0]);
}

void ov90_022591F4(void **p) {
    MessageFormat_ResetBuffers(p[1]);
}

void ov90_02259200(void **p, void *value) {
    BufferPlayersName(p[1], 0, value);
}

void ov90_02259210(void **p, int value) {
    BufferCountryName(p[1], 0, value);
}

void ov90_02259220(void **p, int value, int city) {
    BufferCityName(p[1], 0, value, city);
}

void ov90_02259234(void **p, int value, int width) {
    BufferIntegerAsString(p[1], 0, value, width, 1, 1);
}

void ov90_02259250(void **p, int value) {
    BufferIntegerAsString(p[1], 0, value, 2, 1, 1);
}

void ov90_0225926C(void **p, int value) {
    BufferWiFiPlazaInstrumentName(p[1], 1, value);
}

void ov90_0225927C(void **p, int value) {
    BufferWiFiPlazaActivityName(p[1], 0, value);
}

void ov90_0225928C(void **p, void *dest, int msgNo) {
    ReadMsgDataIntoString(p[0], msgNo, p[3]);
    StringExpandPlaceholders(p[1], dest, p[3]);
}
