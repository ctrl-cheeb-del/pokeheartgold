...
int FrtCmd_023(void *c) {
    void *s = FSYS(c);
    u16 a = FrontierScriptContext_ReadHalfWord(c), b = FrontierScriptContext_ReadHalfWord(c);
    ov80_0222E5A8(PTR(s, 0x60), a, 0xff, b);
    return 0;
}

int FrtCmd_024(void *c) {
    void *s = FSYS(c);
    u16 a = FrontierScriptContext_ReadHalfWord(c), b = FrontierScriptContext_ReadHalfWord(c), d = FrontierScriptContext_ReadHalfWord(c);
    ov80_0222E5A8(PTR(s, 0x60), a, b, d);
    return 0;
}

int FrtCmd_025(void *c) {
    ov80_0222E5B0(PTR(FSYS(c), 0x60));
    FrontierScriptContext_Pause(c, ov80_0222C33C);
    return 1;
}
