...
void ov15_021FF7AC(Window *windows) {
    s32 i;
    for (i = 0; i < 4; i++) {
        ClearWindowTilemapAndScheduleTransfer(&windows[i]);
    }
}

void ov15_021FF7C4(BagWorkRaw *work) {
    AddTextPrinterParameterizedWithColor((Window *)&work->raw[0x1F4], 0, *(String **)&work->raw[0x314], 5, 0, 0xFF, 0x000F0E00, NULL);
    ScheduleWindowCopyToVram((Window *)&work->raw[0x1F4]);
}

void ov15_021FF7FC(BagWorkRaw *work) {
    AddTextPrinterParameterizedWithColor((Window *)&work->raw[0x1F4], 0, *(String **)&work->raw[0x334], 5, 0, 0xFF, 0x000F0E00, NULL);
    ScheduleWindowCopyToVram((Window *)&work->raw[0x1F4]);
}

void ov15_021FF834(BagWorkRaw *work) {
    ClearWindowTilemapAndScheduleTransfer((Window *)&work->raw[0x1F4]);
}

void ov15_021FF844(BagWorkRaw *work) {
    ClearWindowTilemapAndScheduleTransfer((Window *)&work->raw[0x74]);
}
