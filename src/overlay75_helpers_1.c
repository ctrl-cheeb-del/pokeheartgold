...
void ov75_02246BCC(State *p, int a, int b) {
    *(u32 *)((u8 *)p + 0x88) = a;
    *(u32 *)((u8 *)p + 0x8c) = b;
}

void ov75_02246BD8(State *p) {
    *(u32 *)((u8 *)p + 0x88) = 6;
}

void ov75_02246BE0(State *p, int v) {
    *((u8 *)p + 0x7a) = v;
}

u8 ov75_02246BE8(State *p) {
    return *((u8 *)p + 0x7a);
}

void *ov75_02246BF0(int unused, u32 n, int align) {
    OSIntrMode irq = OS_DisableInterrupts();
    void *p = NNS_FndAllocFromExpHeapEx(_02249BE0, n, align);
    OS_RestoreInterrupts(irq);
    return p;
}

void ov75_02246C18(int unused, void *p) {
    if (p) {
        OSIntrMode irq = OS_DisableInterrupts();
        NNS_FndFreeToExpHeap(_02249BE0, p);
        OS_RestoreInterrupts(irq);
    }
}
