...
s32 ov48_022592E0(void *p, void *q) {
    s32 dx;
    s32 dy;
    dx = U32AT(p, 0) - U32AT(q, 0);
    if (dx < 0) {
        dx = -dx;
    }
    dy = U32AT(p, 4) - U32AT(q, 4);
    if (dy < 0) {
        dy = -dy;
    }
    if (dx > GF_DegreeToSinCosIdxNoWrap(0xB4)) {
        dx = 0xFFFF - dx;
    }
    if (dy > GF_DegreeToSinCosIdxNoWrap(0xB4)) {
        dy = 0xFFFF - dy;
    }
    return FX_Sqrt((dx * dx + dy * dy) << 12) >> 12;
}
