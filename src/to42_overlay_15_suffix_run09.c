...
void ov15_021FF894(BagWorkRaw *work) {
    typedef struct {
        u8 pad0[0x248];
        SpriteSystem *system;
    ...
    SpriteSystem_FreeResourcesAndManager(w->system, w->manager);
    SpriteSystem_Free(w->system);
    Heap_Free(w->allocation);
}

void ov15_021FF8D4(BagWorkRaw *work) {
    typedef struct {
        u8 pad[0x250];
        ManagedSprite *sprites[39];
    } Work;
    u32 i;
    for (i = 0; i < 39; i++) {
        ManagedSprite_TickFrame(((Work *)work)->sprites[i]);
    }
}
