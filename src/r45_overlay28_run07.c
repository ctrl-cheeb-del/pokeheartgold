...
void ov28_0225DE04(fx32 *ptr, int deg) {
    ptr[0] = FX_Mul(GF_CosDeg((u16)deg), 0x44000) + 0x54000;
    ptr[1] = FX_Mul(GF_SinDeg((u16)deg), 0x44000) + 0x164000;
}

u16 ov28_0225DE64(int x, int y) {
    return FX_Atan2Idx((y - 100) << FX32_SHIFT, (x - 84) << FX32_SHIFT);
}

u16 ov28_0225DE78(int x, int y) {
    return FX_Atan2Idx(y << FX32_SHIFT, x << FX32_SHIFT);
}
