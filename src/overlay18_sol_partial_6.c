...
void ov18_021F118C(void *p, int idx, int anim) {
    u8 *base = (u8 *)p + 0x670;
    int offset = idx * 4;
    ManagedSprite_SetAnimationFrame(*(ManagedSprite **)(base + offset), 0);
    ManagedSprite_SetAnim(*(ManagedSprite **)(base + offset), anim);
}

BOOL ov18_021F11AC(void *p, int idx) {
    return ManagedSprite_IsAnimated(*(ManagedSprite **)((u8 *)p + 0x670 + idx * 4));
}
