...
void ov113_021E64DC(Scrn *s, void *narc, int id, int heap) {
    s->allocation = GfGfxLoader_GetScrnDataFromOpenNarc(narc, id, 1, &s->header, heap);
    s->width = (u32) * (u16 *)s->header >> 3;
    s->height = (u32) * ((u16 *)s->header + 1) >> 3;
    s->pixels = (u8 *)s->header + 0xC;
}
