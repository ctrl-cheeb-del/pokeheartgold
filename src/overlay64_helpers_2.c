...
void ov64_021E5A88(State *p) {
    volatile u32 *a = (volatile u32 *)0x027e0000;
    DoScheduledBgGpuUpdates(*(void **)((u8 *)p + 4));
    SpriteSystem_TransferOam();
    a[0x3ff8 / 4] |= 1;
}

void ov64_021E5AAC(const void *p, u32 d, u32 n) {
    DC_FlushRange(p, n);
    GX_LoadOBJ(p, d, n);
}

void ov64_021E5AC8(const void *p, u32 d, u32 n) {
    DC_FlushRange(p, n);
    GXS_LoadOBJ(p, d, n);
}

void ov64_021E5AE4(const void *p, u32 d, u32 n) {
    DC_FlushRange(p, n);
    GX_LoadOBJPltt(p, d, n);
}

void ov64_021E5B00(void) {
    GfGfx_SetBanks(ov64_021E6FAC);
}
