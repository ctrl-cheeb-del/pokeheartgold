...
void ov51_021E77A0(TrainerCardMainState *state, int message) {
    ...
    int printer;
    if (message == 0) {
        DrawFrameAndWindow2(&state->data[0xb4], 1, 0x3e1, 0xd);
    }
    FillWindowPixelBuffer(&state->data[0xb4], 0xf);
    slot = (u8 *)state + message * 4;
    printer = AddTextPrinterParameterizedWithColor(&state->data[0xb4], 1, *(void **)&slot[0x33e4], 0, 0, state->data[0x343e], 0x1020f, 0);
    state->data[0x343c] = printer;
