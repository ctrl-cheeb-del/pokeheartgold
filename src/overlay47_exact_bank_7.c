...
void ov47_02259B30(State *p, State *stats, void *unused, int palette) {
    int i;
    State *cursor;
    *(u32 *)((u8 *)p + 0x64) = 0;
    cursor = p;
    for (i = 0; i < 3; i++) {
        *(u32 *)((u8 *)cursor + 0x68) = 0;
        *(u32 *)((u8 *)cursor + 0x74) = ov47_02259DA4(stats, i, 0x50);
        SetWindowPaletteNum((u8 *)p + (i + 2) * 0x10, palette);
        FillWindowPixelBuffer((u8 *)p + (i + 2) * 0x10, 0);
