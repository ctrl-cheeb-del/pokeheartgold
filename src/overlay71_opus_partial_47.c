...
void ov71_0224B970(Ov71Task2 *w) {
    if (w->unk_7C != NULL) {
        ov71_02247424(SysTask_GetData(w->unk_7C));
        SysTask_Destroy(w->unk_7C);
        w->unk_7C = NULL;
    }
}
...
void ov71_0224B990(void *task, Ov71Bright *w) {
    ...
    w->unk_10--;
    if (w->unk_10 > 0) {
        value = w->unk_04 + w->unk_0C;
        w->unk_04 = value;
        G2x_SetBlendBrightness_((vu16 *)0x04000050, 1, value >> 12);
        return;
    }
    G2x_SetBlendBrightness_((vu16 *)0x04000050, 1, w->unk_08 >> 12);
    ov71_0224B970((Ov71Task2 *)w->unk_00);
}
