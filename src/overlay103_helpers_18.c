...
void ov103_021EE094(void *sub) {
    u32 i = 0;
    do {
        ov103_021EE078(sub, i);
        i++;
    } while (i < 7);
}

void ov103_021EE0AC(void *sub, u32 i, u32 anim) {
    u8 *base = (u8 *)sub + S_SPRITES;
    ManagedSprite_SetAnimationFrame(((void **)base)[i], 0);
    ManagedSprite_SetAnim(((void **)base)[i], anim);
}

void ov103_021EE0CC(void *sub, u32 i, u32 on) {
    if (on == 1) {
        ManagedSprite_SetDrawFlag(PAT((u8 *)sub + i * 4, S_SPRITES), 1);
    } else {
        ManagedSprite_SetDrawFlag(PAT((u8 *)sub + i * 4, S_SPRITES), 0);
    }
}

void ov103_021EE0F8(void *sub, u32 i, u32 x, u32 y) {
    ManagedSprite_SetPositionXY(PAT((u8 *)sub + i * 4, S_SPRITES), x, y);
}

void ov103_021EE110(void *sub, u32 i, u32 on) {
    if (on == 1) {
        ManagedSprite_SetOamMode(PAT((u8 *)sub + i * 4, S_SPRITES), 1);
    } else {
        ManagedSprite_SetOamMode(PAT((u8 *)sub + i * 4, S_SPRITES), 0);
    }
}

void ov103_021EE13C(void *sub) {
    void *n = NARC_New(0x14, 0x9d);
    PAT(sub, 0x9d << 2) = n;
}

void ov103_021EE150(void *sub) {
    NARC_Delete(PAT(sub, 0x9d << 2));
}
