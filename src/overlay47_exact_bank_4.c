...
void ov47_02258CC0(State *p) {
    int i = 0;
    State *cursor = p;
    do {
        DestroyMsgData(*(void **)cursor);
        i++;
        cursor = (State *)((u8 *)cursor + 4);
    } while (i < 9);
    MessageFormat_Delete(*(void **)((u8 *)p + 0x24));
    String_Delete(*(void **)((u8 *)p + 0x28));
    String_Delete(*(void **)((u8 *)p + 0x2C));
}

void *ov47_02258CEC(State *p, int i, int msg) {
    if ((u32)i >= 9) {
        GF_AssertFail();
    }
    ReadMsgDataIntoString(*(void **)((u8 *)p + i * 4), msg, *(void **)((u8 *)p + 0x2C));
    StringExpandPlaceholders(*(void **)((u8 *)p + 0x24), *(void **)((u8 *)p + 0x28), *(void **)((u8 *)p + 0x2C));
    return *(void **)((u8 *)p + 0x28);
}

void *ov47_02258D18(State *p, int n) {
    if ((u32)n >= 60) {
        GF_ASSERT((u32)n >= 60);
        n = 0;
    }
    if ((u32)n < 50) {
        return ov47_02258CEC(p, 1, n);
    }
    return ov47_02258CEC(p, 2, n - 50);
}

void *ov47_02258D44(State *p, int n, int base) {
    if ((u32)n >= 60) {
        GF_ASSERT((u32)n >= 60);
        n = 0;
    }
    if ((u32)n < 50) {
        return ov47_02258CEC(p, base + 3, n);
    }
    return ov47_02258CEC(p, base + 6, n - 50);
}

void ov47_02258D78(State *p, int n, int heap) {
    void *profile = PlayerProfile_New(heap);
    void *x = ov45_0222A5C0(*(void **)((u8 *)p + 0x30));
    ov45_0222A844(x, profile, heap);
    BufferPlayersName(*(void **)((u8 *)p + 0x24), n, profile);
    Heap_Free(profile);
}

void ov47_02258DA8(void *a, void *b, void *c, void *d, void *e) {
    ov47_02258DD0(a, b, c, e);
}

void ov47_02258DB4(void *a, void *b, void *c, void *d, void *e, void *f) {
    ov47_02258F48(a, b, c, d, e, f);
}
