...
void ov70_022427C4(Work*w,int dir){int total;if(dir<0){PlaySE(0x5e0);Sprite_SetAnimActiveFlag(*(void**)((u8*)w+0x14),1);Sprite_SetAnimCtrlSeq(*(void**)((u8*)w+0x14),0x27);if(w->page!=0)w->page--;else w->page=2;}else{PlaySE(0x5e0);Sprite_SetAnimActiveFlag(*(void**)((u8*)w+0x10),1);Sprite_SetAnimCtrlSeq(*(void**)((u8*)w+0x10),0x26);if(w->page<2)w->page++;else w->page=0;}ov70_022434C0(w,w->page,w->count);total=ov70_02242508(w->count,4);ov70_02243F00(*(void**)((u8*)w+0x1c),*(void**)((u8*)w+0x38),(u8*)*(void**)((u8*)w+4)+0x40,w->page,total);}

int ov70_02242860(Work*w,int input){int idx;switch(input){case 4:PlaySE(0x5dc);return -2;case 5:ov70_022427C4(w,-1);return -1;case 6:ov70_022427C4(w,1);return -1;}idx=input+w->page*4;if(idx<w->count){PlaySE(0x5dc);return w->items[input].b;}return -1;}
