...
void ov48_0225A1D0(void *p) {
    ClearFrameAndWindow2(ADD(p, 0x1A8), 1);
    ClearWindowTilemapAndScheduleTransfer(ADD(p, 0x1A8));
}

void ov48_0225A1EC(void *p) {
    ScheduleWindowCopyToVram(ADD(p, 0x1A8));
    DrawFrameAndWindow2(ADD(p, 0x1A8), 1, 0xA, 0xA);
}
