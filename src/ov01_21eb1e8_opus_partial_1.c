...
void ov01_021EB1E8(void *textureManager) {
    ((u32 *)textureManager)[0x62] = 1;
}

void *WeatherManager_New(void *fieldSystem) {
    WeatherManager *mgr = Heap_Alloc(4, sizeof(WeatherManager));
    memset(mgr, 0, sizeof(WeatherManager));
    mgr->sys = ov01_021EB64C(fieldSystem);
    mgr->unk04 = 0;
    mgr->unk08 = 0;
    mgr->unk0C = 6;
    mgr->task = NULL;
...
void WeatherManager_Delete(void *weatherManager) {
    WeatherManager *mgr = weatherManager;
    if (mgr->task != NULL) {
        SysTask_Destroy(mgr->task);
    }
    ov01_021EB68C(mgr);
    memset(mgr, 0, sizeof(WeatherManager));
    Heap_Free(mgr);
...
void WeatherManager_SetWeather(void *weatherManager, int a1) {
    ...
    if (mgr->unk04 != a1) {
        if (!ov01_021EB700(mgr->sys, 8, mgr->unk04)) {
            GF_AssertFail();
        }
        if (!ov01_021EB700(mgr->sys, 0, a1)) {
            GF_AssertFail();
        }
        if (!ov01_021EB700(mgr->sys, 3, a1)) {
            GF_AssertFail();
        }
        mgr->unk04 = a1;
    }
...
BOOL WeatherManager_ChangeWeather(void *weatherManager, int weatherType) {
    ...
    if (mgr->unk0C != 6) {
        mgr->unk10 = weatherType;
        return TRUE;
    }
    if (mgr->unk04 == weatherType) {
        return TRUE;
    }
    if (mgr->task != NULL) {
        GF_AssertFail();
    }
    mgr->unk08 = weatherType;
    if (!ov01_021EB4B4(mgr->unk04, weatherType)) {
        mgr->unk0C = 0;
        mgr->task = SysTask_CreateOnMainQueue(ov01_021EB320, mgr, 0);
    } else {
        mgr->unk0C = 0;
        mgr->task = SysTask_CreateOnMainQueue(ov01_021EB3F0, mgr, 0);
    }
    return TRUE;
}
