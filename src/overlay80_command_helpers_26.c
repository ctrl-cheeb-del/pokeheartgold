...
int FrtCmd_065(void *c) {
    void *p = sub_0209680C(APP(c));
    u16 a = FrontierScript_ReadVar(c), b = FrontierScript_ReadVar(c), d = FrontierScript_ReadVar(c);
    ov80_022399A4(PTR(p, 0x10), a, b, d);
    return 0;
}

int FrtCmd_066(void *c) {
    void *p = sub_0209680C(APP(c));
    ov80_02239A1C(PTR(p, 0x10), FrontierScript_ReadVar(c));
    return 0;
}

int FrtCmd_067(void *c) {
    void *p = sub_0209680C(APP(c));
    u16 a = FrontierScript_ReadVar(c), b = FrontierScript_ReadVar(c);
    sub_02015494(ov80_02239A60(PTR(p, 0x10), a), b, 0, 0);
    return 0;
}

int FrtCmd_068(void *c) {
    FrontierScriptContext_Pause(c, ov80_0222DE00);
    return 1;
}

BOOL ov80_0222DE00(void *c) {
    return ov80_02239A74(PTR(sub_0209680C(APP(c)), 0x10)) == 1;
}
