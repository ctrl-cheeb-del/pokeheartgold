...
int FrtCmd_016(void *c) {
    u16 v = FrontierScriptContext_ReadHalfWord(c);
    ov80_0222E268(FSYS(c), PTR(c, 0x80), v, 0, 0);
    FrontierScriptContext_Pause(c, ov80_0222C17C);
    return 1;
}

int FrtCmd_017(void *c) {
    u16 v = FrontierScriptContext_ReadHalfWord(c);
    ov80_0222E268(FSYS(c), PTR(c, 0x80), v, 1, 0);
    FrontierScriptContext_Pause(c, ov80_0222C17C);
    return 1;
}

BOOL ov80_0222C17C(void *c) {
    if (!TextPrinterCheckActive(U8(FSYS(c), 0x50))) {
