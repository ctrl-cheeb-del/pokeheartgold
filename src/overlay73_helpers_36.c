...
int ov73_021EA12C(void *p) {
    return sub_0203A880(p);
}

void ov73_021EA134(void *p, u32 n, u32 v) {
    if (n <= 1) {
        Sprite_SetDrawFlag(PTR(p, 0xdd0), v);
        Sprite_SetDrawFlag(PTR(p, 0xdd4), v);
    }
}

void ov73_021EA15C(void *p) {
    if (!PTR(p, 0xf0c)) {
        PTR(p, 0xf0c) = WaitingIcon_New((u8 *)p + 0xdd8, 1);
    }
}

void ov73_021EA180(void *p) {
    if (PTR(p, 0xf0c)) {
        sub_0200F450(PTR(p, 0xf0c));
        PTR(p, 0xf0c) = 0;
    }
}
