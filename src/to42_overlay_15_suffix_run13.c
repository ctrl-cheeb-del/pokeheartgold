...
void ov15_022000F4(BagWorkRaw *work) {
    ...
    Context *ctx = *(Context **)&work->raw[0x234];
    Entry *entry = (Entry *)((u8 *)ctx + ctx->current * 12);
    if (entry->value <= 6) {
        ManagedSprite_SetDrawFlag(*(ManagedSprite **)&work->raw[0x294], FALSE);
        ManagedSprite_SetDrawFlag(*(ManagedSprite **)&work->raw[0x298], FALSE);
    } else {
        ManagedSprite_SetDrawFlag(*(ManagedSprite **)&work->raw[0x294], TRUE);
        ManagedSprite_SetDrawFlag(*(ManagedSprite **)&work->raw[0x298], TRUE);
    }
}
