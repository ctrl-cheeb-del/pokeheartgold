...
void ov15_021FFEC0(BagWorkRaw *work) {
    work->raw[0x648] = 0;
}

void ov15_021FFECC(BagWorkRaw *work, s32 index) {
    u32 offset = index * 4;
    ManagedSprite_SetPositionXYWithSubscreenOffset(*(ManagedSprite **)&work->raw[0x2A0], ov15_02200AB8[offset], ov15_02200AB9[offset], 1 << 20);
    ManagedSprite_SetAnim(*(ManagedSprite **)&work->raw[0x2A0], ov15_02200ABA[offset]);
    ManagedSprite_SetPaletteOverride(*(ManagedSprite **)&work->raw[0x2A0], ov15_02200ABB[offset]);
    ManagedSprite_SetDrawFlag(*(ManagedSprite **)&work->raw[0x2A0], TRUE);
}

void ov15_021FFF24(BagWorkRaw *work) {
    ManagedSprite_SetDrawFlag(*(ManagedSprite **)&work->raw[0x2A0], FALSE);
}
