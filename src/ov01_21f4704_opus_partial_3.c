...
void ov01_021F4AAC(MapLoadManager *mapLoadManager, MapLoadedChunk *chunk) {
    ...
    NARC_ReadFile(mapLoadManager->narc, 4, &chunk->unk86C);
    fileId = chunk->unk86C.unk00 >> 16;
    if (fileId != 0) {
        NARC_ReadFile(mapLoadManager->narc, fileId, chunk->unk86C.unk04);
    }
}
...
void ov01_021F4AE4(MapLoadManager *mapLoadManager, MapLoadedChunk *chunk, int fileId) {
    ...
    NARC_ReadFile(mapLoadManager->narc, fileId, chunk);
    if (chunk->unkA70 != NULL) {
        for (i = 0; i < 0x400; i++) {
            chunk->unkA70[i] = ((u16 *)chunk)[i];
        }
    }
}
