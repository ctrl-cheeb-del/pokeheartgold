...
void ov59_022398A0(u8 *state) {
    int i = 0;
    u8 *window = state + 0x128;
    for (; i < 18; i++, window += 0x10) {
        ClearWindowTilemapAndCopyToVram(window);
        RemoveWindow(window);
    }
    YesNoPrompt_Destroy(*(void **)(state + 0x248));
}

void ov59_022398CC(u8 *state) {
    int i;
    if (state[0x44] != 0) {
        int offset = (state[0x44] - 1) * 8;
        const ListDesc59 *desc = (const ListDesc59 *)((const u8 *)ov59_0223C668 + offset);
        *(void **)(state + 0x294) = ListMenuItems_New(*(const u16 *)((const u8 *)ov59_0223C668 + offset), *(u32 *)state);
        for (i = 0; i < desc->count; i++) {
            ListMenuItems_AppendFromMsgData(*(void **)(state + 0x294), *(void **)(state + 0x5C), desc->messages[i], i);
        }
    }
}
