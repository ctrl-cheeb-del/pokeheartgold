...
void *ov31_0225E95C(u8 *args, void *msgData) {
    ...
    kind += 0xfd;
    if (kind <= 1) {
        amount = PokeathlonSave_GetAthletePoints(*(void **)&args[0x254]);
    } else {
        amount = PlayerProfile_GetMoney(*(void **)&args[0x248]);
    }
    result = ov03_02257814(args, amount);
    if (result == 2) {
        return NewString_ReadMsgData(msgData, 0x19);
    }
    if (result == 3) {
        return NewString_ReadMsgData(msgData, 0x1a);
    }
    kind = args[0x283];
    kind += 0xfd;
    if (kind <= 1) {
        return NewString_ReadMsgData(msgData, 0x30);
    }
    return NewString_ReadMsgData(msgData, 0xb);
}
...
void *ov31_0225E9CC(const u8 *args, void *msgData) {
    ...
    if (args[0x283] == 0) {
        msgNo = 0x10;
    } else if (args[0x283] == 1) {
        msgNo = 0x16;
    } else if (args[0x283] == 2) {
        msgNo = 0x18;
    } else if (args[0x283] == 3) {
        msgNo = 0x19;
    } else if (args[0x283] == 4) {
        msgNo = 0x1a;
    }
    return NewString_ReadMsgData(msgData, msgNo);
}
