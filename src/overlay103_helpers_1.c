...
u32 ov103_021EC940(void *a0) {
    void *w;
    Heap_Create(3, 0x9c, 0x10000);
    w = OverlayManager_CreateAndGetData(a0, 0x2c, 0x9c);
    MI_CpuFill8(w, 0, 0x2c);
    PAT(w, W_ARGS) = OverlayManager_GetArgs(a0);
    PAT(w, W_OPTIONS) = Save_PlayerData_GetOptionsAddr(PAT(PAT(w, W_ARGS), 0));
    PAT(w, W_MAILBOX) = Save_Mailbox_Get(PAT(PAT(w, W_ARGS), 0));
    U32AT(w, W_STATE) = 8;
    return 1;
}

u32 ov103_021EC988(void *a0, void *a1) {
    void *w = OverlayManager_GetData(a0);
    if (ov103_021ED2D4(w, (u32 *)a1) == 0) {
        return 1;
    }
    return 0;
}
