...
void ov31_0225D60C(Overlay31State *state) {
    *(void **)&state->data[0x154] = MessageFormat_New_Custom(8, 0x40, 8);
    *(void **)&state->data[0x158] = NewMsgDataFromNarc(0, 0x1b, 0x1b3, 8);
    *(void **)&state->data[0x15c] = NewMsgDataFromNarc(0, 0x1b, 0xde, 8);
    *(void **)&state->data[0x188] = String_New(0x90, 8);
}

void ov31_0225D654(Overlay31State *state) {
    String_Delete(*(void **)&state->data[0x188]);
    DestroyMsgData(*(void **)&state->data[0x15c]);
    DestroyMsgData(*(void **)&state->data[0x158]);
    MessageFormat_Delete(*(void **)&state->data[0x154]);
}
