...
void *ov01_021F65D0(MapLoadManager *mapLoadManager, u8 idx) {
    return mapLoadManager->maps[idx]->unk85C;
}

u16 *ov01_021F65E4(MapLoadManager *mapLoadManager, u8 idx) {
    return (u16 *)mapLoadManager->maps[idx];
}

u8 *ov01_021F65F0(MapLoadManager *mapLoadManager, u8 idx) {
    return mapLoadManager->maps[idx]->unkA70;
}

SoundplateStruct *ov01_021F6600(MapLoadManager *mapLoadManager, u8 idx) {
    return (SoundplateStruct *)&mapLoadManager->maps[idx]->unk86C;
}

void ov01_021F6614(SysTask *task) {
    MapLoadTask *data = SysTask_GetData(task);
    data->unk20 = 1;
}
