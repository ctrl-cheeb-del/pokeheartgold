...
int FrtCmd_056(void *c) {
    u16 *out = FrontierScript_ReadVarPtr(c);
    u16 n = FrontierScript_ReadVar(c);
    *out = LCRandom() % n;
    return 1;
}

int FrtCmd_057(void *c) {
    HealParty(SaveArray_Party_Get(PTR(Frontier_GetLaunchArgs(APP(c)), 8)));
    return 0;
}

int FrtCmd_058(void *c) {
    FrontierScriptContext_Pause(c, ov80_0222D2AC);
    return 1;
}
