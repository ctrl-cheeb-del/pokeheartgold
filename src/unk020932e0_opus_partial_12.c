...
void sub_020950C0(UnkStruct_020932E0 *ptr) {
    if (ptr->count == 1) {
        sub_0209501C(ptr, 5, 1);
    }
}

void sub_020950D4(UnkStruct_020932E0 *ptr) {
    if (ptr->count == 1) {
        if (GetWindowBgConfig(&ptr->window1) != NULL) {
            RemoveWindow(&ptr->window1);
        }
    }
}
...
void sub_0209511C(UnkStruct_020932E0 *ptr) {
    ...
    for (i = 0; i < ptr->count; i++) {
        ptr->pairs[i].unk_00 = ptr->rows[i].unk_00;
        ptr->pairs[i].unk_04 = ptr->rows[i].unk_04;
    }
}
