...
void ov05_0221E9C4(Overlay05State *state) {
    FillWindowPixelRect(&state->data[0xb88], 0xf, 0, 0, 0xd8, 0x20);
    DrawFrameAndWindow2(&state->data[0xb88], 0, 1, 0xf);
}

int ov05_0221E9F8(Overlay05State *state) {
    void *p = *(void **)(*(u8 **)(*(u8 **)&state->data[0]) + 0x1c0);
    if (p == NULL) {
        goto no;
    }
    if (sub_0202FC48(p) != 0) {
...
void ov05_0221EA18(Overlay05State *state) {
    if (*(void **)&state->data[0xbc4] == NULL) {
        *(void **)&state->data[0xbc4] = WaitingIcon_New(&state->data[0xb88], 1);
    }
}

void ov05_0221EA38(Overlay05State *state) {
    if (*(void **)&state->data[0xbc4] != NULL) {
        sub_0200F450(*(void **)&state->data[0xbc4]);
        *(void **)&state->data[0xbc4] = NULL;
    }
}
