...
void ov93_0225CEA0(void *p) {
    ...
    ov93_0225EA50(p, PAT(p, 0xd4));
    GF_RunVramTransferTasks();
    SpriteSystem_TransferOam();
    PaletteData_PushTransparentBuffers(PAT(p, 0x8c));
    if (render->showBg == 1) {
        ToggleBgLayer(7, 1);
        render->showBg = 0;
    } else if (render->hideBg == 1) {
        ToggleBgLayer(7, 0);
        render->hideBg = 0;
    }
    DoScheduledBgGpuUpdates(PAT(p, 0x2c));
    base = (volatile u32 *)0x027e0000;
    base[0x3ff8 / 4] |= 1;
}

void ov93_0225CF14(u32 heapId) {
    GF_3DVramMan_Create(heapId, 0, 1, 0, 2, ov93_0225CF34);
}
