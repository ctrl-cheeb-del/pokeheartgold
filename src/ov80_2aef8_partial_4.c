...
BOOL ov80_0222B8D8(void *ctx, u16 value) {
    u8 *p = ctx;
    *(u16 *)(p + 0x3C0) = value;
    return sub_02037030(0x2E, p + 0x3C0, 0x28) == 1;
}
...
BOOL ov80_0222B920(void *ctx, u16 value) {
    u8 *p = ctx;
    *(u16 *)(p + 0x3C0) = value;
    return sub_02037030(0x2F, p + 0x3C0, 0x28) == 1;
}
...
BOOL ov80_0222B968(void *ctx) {
    u8 *p = ctx;
    s32 count = ov80_02237B24(p[0x10], 0);
    ...
    if (count > 0) {
        u32 offset = i;
        do {
            void *mon = Party_GetMonByIndex(*(void **)(p + 0x28), i);
            MI_CpuCopy8(mon, p + 0x410 + offset, size);
            i++;
            offset += size;
        } while (i < count);
    }
    return sub_02036FD8(0x30, p + 0x410, 0x200) == 1;
}
