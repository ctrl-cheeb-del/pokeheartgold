...
void ov48_02259030(void *p) {
    if (U16AT(p, 2) == 0) {
        ov48_0225A158(ADD(p, 0xC40C), ADD(p, 0x168), ADD(p, 0x20));
    }
}

void ov48_02259050(void *p) {
    Thunk_G3X_Reset();
    ov48_02259C44(ADD(p, 0xC3CC));
    ov48_02259874(ADD(p, 0x178));
    ov48_022599A0(ADD(p, 0x224), ADD(p, 0x178));
    ov48_022594D0(ADD(p, 0x20));
    RequestSwap3DBuffers(0, 1);
}

void ov48_02259090(void *p) {
    ov48_022594DC(ADD(p, 0x20));
}
