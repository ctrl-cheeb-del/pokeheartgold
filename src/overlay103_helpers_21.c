...
void ov103_021EE784(void *a0, u32 unused) {
    void *mail = PAT((u8 *)PAT(a0, W_SUB) + U8AT(a0, W_SLOT) * 4, S_MAIL);
    void *s = String_New(8, 0x9d);
    CopyU16ArrayToString(s, Mail_GetAuthorNamePtr(mail));
    BufferString(PAT(PAT(a0, W_SUB), S_MSGFMT), 0, s, Mail_GetAuthorGender(mail), 1, 2);
    String_Delete(s);
}
