...
int ov31_0225E774(Overlay31State *state) {
    int result;
    if (*(void **)&state->data[0x170] == NULL) {
        return 0;
    }
    result = YesNoPrompt_HandleInputForSave(*(void **)&state->data[0x170]);
    switch (result) {
    case 1:
        YesNoPrompt_Destroy(*(void **)&state->data[0x170]);
        *(void **)&state->data[0x170] = NULL;
        *(u32 *)(*(u8 **)&state->data[0x14] + 0x294) = 0;
        return 1;
    case 2:
        YesNoPrompt_Destroy(*(void **)&state->data[0x170]);
        *(void **)&state->data[0x170] = NULL;
        *(u32 *)(*(u8 **)&state->data[0x14] + 0x294) = 1;
        return 1;
    }
    return 0;
}
