...
static inline void *FrontierObject(FrontierScriptContextPrivate *ctx) {
    return *ctx->frontier;
}

static inline BOOL LaunchArgsApp(FrontierScriptContextPrivate *ctx, const void *template) {
    FrontierLaunchArgsPrivate *launchArgs = Frontier_GetLaunchArgs(FrontierObject(ctx));
    FrontierDataPrivate *data = Frontier_GetData(FrontierObject(ctx));
    AppArgsPrivate *args = Heap_Alloc(11, sizeof(AppArgsPrivate));
    MI_CpuFill8(args, 0, sizeof(AppArgsPrivate));
    args->saveData = launchArgs->saveData;
    ov80_02231E4C(args, data);
    Frontier_LaunchApplication(FrontierObject(ctx), template, args, 0, ov80_02231E94);
    return TRUE;
}
